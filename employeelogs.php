...
$demp=mysql_fetch_object(mysql_query("select * from dept where ndex='".$emp->deptId."'"));
for ( $i = $start; $i <= $end; $i += 86400 ){
		...
		while($ls2=mysql_fetch_object($ees2)){
			...
			if($_POST['cb'.$ls2->hrint_id]=='on'){
				...
				if($jcounter>1){
					...
					if($old_d['log']!=$new_d){
					$updatelogs_rec=mysql_query("insert into hrinterface_edited (`ipaddress`, `username`, `olddata`, `newdata`, `dyt`,`hrint`) VALUES ('".$_SERVER['REMOTE_ADDR']."','".$_SESSION['fullName']."','".$old_d['log']."','".$ls2->datelog." ".$_POST['inp'.$ls2->hrint_id]."','".date('Y-m-d H:i:s')."','".$ls2->hrint_id."')");
					}
	...
	//echo "select * from hrinterface where dtrid='".$emp->biometricNo."' and datelog>'".$datelog."' and in_out=0 ORDER BY log LIMIT 0,1<br>";
...
//	echo $emp->biometricNo."dddd";
	...
	if($log->leaveId>0 && $log->leaveId!=12){
	...
	if($log->isDayOff>0){ 
		...
		$t_approvedOvertime+=$log->approvedOvertime;
		$t_otnyt+=$otnyt;
		$data.="<tr style='background-color:".$bgclr1s.";' onclick=\"Effect.toggle('".$datelog."', 'blind', { duration: 1.0 });\">
					<td>".$datelog."(".date('D',strtotime($datelog)).")</td>
					<td>".$shiftsked->shft."&nbsp;&nbsp;&nbsp;".$hol."</td>
	...
	if($log->isError==0 && $log->days_absent==0 && $log->leaveId==0 && $log->isDayOff==0 && $log->leaveId!=5 && $log->leaveId!=13){
		$totalnytpremium+=$log->night_prem + $log->approvedOvertimeNightPremium;
		$otnyt=$log->night_prem + $log->approvedOvertimeNightPremium;
		$t_hoursDuty+=$log->hoursDuty;
		$t_minutesLate+=$log->minutesLate;
		$t_undertime+=$log->undertime;
		$t_approvedOvertime+=$log->approvedOvertime;
		$t_otnyt+=$otnyt;
		$data.="<tr style='background-color:".$bgclr1s.";' onclick=\"Effect.toggle('".$datelog."', 'blind', { duration: 1.0 });\">
					<td>".$datelog."(".date('D',strtotime($datelog)).")</td>
...
//	echo "select * from hrinterface where dtrid=".$emp->biometricNo." and hrint_id in (".$hrinterfaceid.")<br>";
...
if($_SESSION['ndex']==12 || $_SESSION['ndex']==14 || $_SESSION['ndex']==16 || $_SESSION['ndex']==17 || $_SESSION['ndex']==21 || $_SESSION['ndex']==22 || $_SESSION['ndex']==336 || $_SESSION['ndex']==273) {  
	$data.="<tr>
				<td colspan='10'>
...
//		echo "SELECT * from employee_leave where employeeId='".$_GET['id']."' and leaveId='".$lev->ndex."' where startDate>='".date($yir.'-01-01')."' and endDate<='".date($yir.'-12-31')."' order by startDate";
		...
		$lend = strtotime($l->endDate);
			for ( $la = $lstart; $la <= $lend; $la += 86400 ){
				$lrd="(";
				$lrestdayq=mysql_query("select * from employee_restday where employeeId=".$_GET['id']." and startDate<'".date('Y-m-d',$la)."' ORDER BY startDate DESC LIMIT 2 ");
...
<table style="font-family:Arial;font-size:12px;">
	...
	&nbsp;&nbsp;-&nbsp;&nbsp;<select style="font-size:11px;" name="newemp" onchange="changeurl(this.value,<?php echo $defco;?>)"><?php echo $optionemployee;?></select><br>
	<font style="font-size:12px;font-weight:bold;">Dept: <?php echo $demp->name;?></font>
...
<form name="wee" id="wee" action="employeelogs.php?act=ot&mants=<?php echo $defco;?>&id=<?php echo $_GET['id'];?>&lastdate=<?php echo $lastdate;?>&lasthrisid=<?php echo $lasthrisid;?>" method="post">
<table style="font-family:Arial;font-size:12px;" width="100%">
	<tr>
...
<tr>
<td colspan="4" align="right"><?php echo $t_hoursDuty;?></td>
<td align="right"><?php echo $t_minutesLate;?></td>
<td align="right"><?php echo $t_undertime;?></td>
<td align="right"><?php echo $t_approvedOvertime;?></td>
<td align='right'><?php echo number_format($totalnytpremium,2);?></td></tr>
			</table>
		</td>
