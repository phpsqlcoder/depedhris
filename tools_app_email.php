...
if($_GET['act']=='aprob'){
	...
	foreach($types as $type){
		...
		$qry.=" and k.tayp='".$type."' and k.date>='".$_GET['startDate']."' and k.date<='".$_GET['endDate']."' and k.approve1='".$_GET['aStatus']."'";
	if($_GET['aStatus']=='0'){
		...
		while($r=mysql_fetch_object($exec)){
			...
			if($log['ndex']){
			    ...
			    $date_approved='';
			    $edit_link="<a href='#'	class='btn btn-xs red' onclick=\"window.open('online_apps/".$link."?id=".$r->ndex."','displayWindow','toolbar=no,scrollbars=yes,width=1200,height=500')\";> Edit</a>";
			  }
