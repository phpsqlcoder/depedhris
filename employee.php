...
#############################
$statusqry=mysqli_query($conn,"select * from employeechangestatus where isServed=0 and effectivityDate<='".date('Y-m-d')."'");
while($st=mysqli_fetch_object($statusqry)){
    if($st->changeType=='Lateral Transfer' || $st->changeType=='Promotion' || $st->changeType=='Demotion'){
        if($st->isDept==1){
            $upddept=mysqli_query($conn,"update employee set deptId='".$st->newValue."' where ndex='".$st->employeeId."'");
        }
        else{
            $updstatus=mysqli_query($conn,"update employee set position='".$st->newValue."' where ndex='".$st->employeeId."'");
        }
    }
    elseif($st->changeType=='Employment Status'){
        ...
        $lastempno=mysqli_fetch_object(mysqli_query($conn,"SELECT employeeNo FROM employee WHERE employmentStatus in ".$statuscondition." ORDER BY employeeNo DESC"));
        $newEmployeeNumber=$lastempno->employeeNo + 1;
        $emprec=mysqli_fetch_object(mysqli_query($conn,"SELECT * FROM employee WHERE ndex='".$st->employeeId."'"));
        if($emprec->employmentStatus=='Temporary' && $st->newValue=='Probationary'){            
            $updatedatestart=mysqli_query($conn,"update employee set dateHired='".$st->effectivityDate."' where ndex='".$st->employeeId."'");
        }
        ...
        $updstatus=mysqli_query($conn,"update employee set employmentStatus='".$st->newValue."',employeeNo='".$newEmployeeNumber."' where ndex='".$st->employeeId."'");
    }
    elseif($st->changeType=='Resignation' || $st->changeType=='Retirement' || $st->changeType=='End of Contract' || $st->changeType=='Termination' || $st->changeType=='Separation'){
        $updstatus=mysqli_query($conn,"update employee set endDate='".$st->effectivityDate."' where ndex='".$st->employeeId."'");
    }
elseif($st->changeType=='deptId' || $st->changeType=='position' || $st->changeType=='employmentStatus'){
        $updstatus=mysqli_query($conn,"update employee set ".$st->changeType."='".$st->newValue."' where ndex='".$st->employeeId."'");
    }
    if($st->changeType=='Resignation' || $st->changeType=='Retirement' || $st->changeType=='End of Contract' || $st->changeType=='End of Residency Training' || $st->changeType=='Termination' || $st->changeType=='Separation'){
        $updEmpStatus=mysqli_query($conn,"update employee set isActive=0 where ndex=".$st->employeeId."");
    }
    $updstattable=mysqli_query($conn,"update employeechangestatus set isServed=1 where ndex=".$st->ndex."");
}

$endqry=mysqli_query($conn,"UPDATE employee set isActive='0' WHERE isActive<>'0' and (endDate<CURDATE() and endDate<>'0000-00-00')");
############################
...
$qry.=" ORDER BY e.lastName,e.firstName,e.middleName";
 ...
 if($_SESSION['isViewingaccess']==0) {  
	...
	elseif($_SESSION['ndex']=='422'){
        ...
        elseif($_SESSION['ndex']=='114'  || $_SESSION['ndex']==398 || $_SESSION['ndex']==418 || $_SESSION['ndex']==430 || $_SESSION['ndex']==432){
        $data.="<tr style='background-color:".$bgclr1s.";font-size:12px;'>
                <td>".getID($employmentStatus,$employeeNo)."</td>
                <td><strong>".$lastName.", ".$firstName."&nbsp;".$middleName."</strong></td>
                <td> ".$department."</td>
                <td><a href='#' title='Edit Employee 201' onclick=\"window.location.href='employee_edit.php?id=".$ndex."'\";><img src=\"images/edit.png\" height='15' width='15'></a>&nbsp;&nbsp;
                <a href='#' title='Daily Time Record' onclick=\"window.open('employeelogs.php?id=".$ndex."','displayWindow','toolbar=no,scrollbars=yes,width=1110,height=500')\";><img src=\"images/view.png\" height='15' width='15'></a>&nbsp;&nbsp;
                <a href='#' title='Print Contract' onclick=\"window.open('employeecontract.php?id=".$ndex."','displayWindow','toolbar=no,scrollbars=yes,width=1000,height=800')\";><img src=\"images/usercontract.png\" height='15' width='15'></a>&nbsp;&nbsp;
                <a href='#' title='Change Employment Status' onclick=\"window.open('employeechangestatus.php?id=".$ndex."','displayWindow','toolbar=no,scrollbars=yes,width=600,height=500')\";><img src=\"images/employmentstatus.png\" height='15' width='15'></a>&nbsp;&nbsp;
<a href='#' title='Code of Discipline' onclick=\"window.open('employeecod.php?id=".$ndex."','displayWindow','toolbar=no,scrollbars=yes,width=600,height=500')\";><img src=\"images/cod.png\" height='15' width='15'></a>&nbsp;&nbsp;
                <a href='#' title='Medical Record' onclick=\"window.open('employeemedical.php?id=".$ndex."','displayWindow','toolbar=no,scrollbars=yes,width=800,height=500')\";><img src=\"images/icondock/mkexp.gif\" height='15' width='15'></a>&nbsp;&nbsp;</td>
                </tr>";
        }
        elseif($_SESSION['ndex']=='236' || $_SESSION['ndex']=='326' || $_SESSION['ndex']=='427'){
        $data.="<tr style='background-color:".$bgclr1s.";font-size:12px;'>
                <td>".getID($employmentStatus,$employeeNo)."</td>
                <td><strong>".$lastName.", ".$firstName."&nbsp;".$middleName."</strong></td>
                <td> ".$department."</td>
        ...
        elseif($_SESSION['ndex']=='325'){
        $data.="<tr style='background-color:".$bgclr1s.";font-size:12px;'>
                <td>".getID($employmentStatus,$employeeNo)."</td>
                <td><strong>".$lastName.", ".$firstName."&nbsp;".$middleName."</strong></td>
                <td> ".$department."</td>
                <td>
<a href='#' title='Edit Employee 201' onclick=\"window.location.href='employee_edit.php?id=".$ndex."'\";><img src=\"images/edit.png\" height='15' width='15'></a>&nbsp;&nbsp;
                <a href='#' title='Daily Time Record' onclick=\"window.open('employeelogs.php?id=".$ndex."','displayWindow','toolbar=no,scrollbars=yes,width=1110,height=500')\";><img src=\"images/view.png\" height='15' width='15'></a>&nbsp;&nbsp;
...
<!-- <div id="main_content_wrap" class="container_12"> -->
        <h2>Employees &nbsp;&nbsp;&nbsp;&nbsp;  <?php if($_SESSION['isViewingaccess']==0 || $_SESSION['ndex']=='114' || $_SESSION['ndex']==398 || $_SESSION['ndex']=='179' || $_SESSION['ndex']=='154' || $_SESSION['ndex']=='236' || $_SESSION['ndex']=='259' ||  $_SESSION['ndex']=='336' ||  $_SESSION['ndex']=='325' ||  $_SESSION['ndex']=='427' ||  $_SESSION['ndex']=='430' ||  $_SESSION['ndex']=='432' ||  $_SESSION['ndex']=='451' ||  $_SESSION['ndex']=='452' ||  $_SESSION['ndex']=='453') { ?>  <button onclick="window.open('employee_add.php','_self');">Add Employee</button><?php } ?></h2>
