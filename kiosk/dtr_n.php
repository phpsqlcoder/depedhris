...
if($_POST['mant']){
	...
	$yeer = date('Y',strtotime($getco->payrollDate));
...
$levqry=mysql_query("select * from `leave` where ndex not in(5,13) order by name");
	while($lev=mysql_fetch_object($levqry)){
		...
		$levtotalqry=mysql_query("SELECT * from employee_leave where employeeId='".$_GET['id']."' and leaveId='".$lev->ndex."' and startDate>='".$yeer."-01-01' and endDate<='".$yeer."-12-31' order by startDate");
		...
		$levlimit=mysql_fetch_object(mysql_query("select * from employee_leave_limit where employeeId='".$_GET['id']."' and leaveId='".$lev->ndex."' and yer='".$yeer."'"));
...
<table width="100%">
	<tr>
		...
		<td valign="top"><table style="font-family:Arial;font-size:12px;">
			...
			<br><br>
			<table style="font-family:Arial;font-size:12px;">
				<tr style="color:blue;font-weight:bold;"><td>Dates with Failure to Log</td></tr>			
				<?php
					$del = mysql_query("select * From hrinterface_deleted where dtrid='".$emp->biometricNo."' and datelog>='".$startdate."' and datelog<='".$enddate."'");

					while($de=mysql_fetch_array($del)){
						echo '<tr><td>'.$de['datelog'].'</td></tr>';
					}
