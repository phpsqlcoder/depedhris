...
if($_GET['act']=='adnew'){
  ...
  $u=mysql_query("insert into kiosk_slide (`seq`, `image`)values('".$_POST['a']."','".basename($_FILES["b"]["name"])."')");
  header("location:kiosknewslide.php");
}
if($_GET['acts']=='delete'){
	$del=mysql_query("delete from kiosk_slide where ndex='".$_GET['id']."'");
	header("location:kiosknewslide.php");
}

$dpqry=mysql_query("select * from kiosk_slide ORDER by seq");
while($d=mysql_fetch_object($dpqry)){
	$data.="<tr>
		<td>".$d->seq."</td>
		<td><a href='excel/".$d->image."' target='_blank'><img src='excel/".$d->image."' height='50'></a></td>		
		<td><a href='kiosknewslide.php?acts=delete&id=".$d->ndex."'>Remove</a></td>
	</tr>";
