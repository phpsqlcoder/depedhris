...
$sql = "SELECT * FROM loansetup WHERE employeeId='".$_GET['id']."'";// && posted='1'";
//echo $sql;
$rs = mysql_query($sql);
while ($dt = mysql_fetch_assoc($rs)){
	$loanAmount = $dt['loanAmount'];
	$rs1 = mysql_query("SELECT * FROM loanpayments WHERE loanSetupId='".$dt['ndex']."'");
	...
	$viewResult .="<tr onclick=\"Effect.toggle('".$dt['ndex']."', 'blind', { duration: 1.0 });\" style='background-color:#ffff99;font-size:12px;'><td>".$dt['loanType']."</td><td>".number_format($dt['loanAmount'],2)."</td><td>".$dt['nOfDeduction']."</td><td>".$dt['dedDateStart']."</td><td>".number_format($dt['dedAmount'],2)."</td><td>".number_format($dt['loanBalance'],2)."</td></tr>
					<tr>
						<td colspan='5'>
