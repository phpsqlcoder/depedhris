...
$qry.=" order by di.name,d.name,e.lastname, e.firstname, e.middleName";
//echo $qry;
...
$qryex=mysql_query($qry);
	...
	while($s=mysql_fetch_object($qryex)){
		...
		for ( $i = $start; $i <= $end; $i += 86400 ){
			...
			$day=mysql_fetch_array(mysql_query("select * from dailytimesummary where employeeId='".$s->emp."' and date='".$datelog."'"));
			$data.="<td>".$day['hoursDuty']."</td>";
		}
...
?>
	...
	?>
	 ...
	 <tr><td>&nbsp;</td></tr>
	  <tr>
	       <td colspan="7" align="center" style="font-size:14px;font-weight:bold;">Total Duty per Employee (Detailed)</td>
	  </tr>
	  <tr><td>&nbsp;</td></tr>
	  <tr style="font-weight:bold;">
		   ...
		   $start = strtotime($_POST['startdate']);
			...
			for ( $i = $start; $i <= $end; $i += 86400 ){
				$yy++;
				echo "<td>".date('Y-m-d',$i)."</td>";
			}
		   ?>
		   <td>Total no. of days</td>
	  </tr>
	  <tr><td colspan="<?php echo $yy;?>"><hr></td></tr>
	  <?php echo $data;?>
      </table>
