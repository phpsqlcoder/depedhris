...
$qry="SELECT k.*,e.firstName,e.lastName,e.middleName,p.name as position,d.name as dept,e.employmentStatus,e.employeeNo,e.biometricNo 
...
$qry.=" WHERE (k.date >= '".$cutoffDate['cutoffDateStart']."' and k.date <= '".$cutoffDate['cutoffDateEnd']."') and k.tayp='log' ".$cond."";
...
while($xp=mysql_fetch_array($exec)){
	...
	$approved_hr=mysql_fetch_array(mysql_query("select * from kiosk_request_logs where request_id='".$xp['ndex']."' and action='Approve Request (HR)' order by ndex desc limit 1"));
   	...
   	$ll=mysql_fetch_array(mysql_query("select * from hrinterface where 
   		dtrid='".$xp['biometricNo']."' and datelog='".$xp['date']."'"));
   	if(!$ll['ndex']){
	     $var++;
	     $ctr1s++;
...
if($_POST['eksel']=='on'){
	...
	?>
     ...
     <table width="100%" style="font-family:Arial;font-size:12px;">
	  <tr>
	       <td colspan="11" align="center" style="font-size:14px;font-weight:bold;">Forgot to Log Application<br> 
