...
function checkDay1($dyt,$id){	
	
	$eleave=mysql_fetch_object(mysql_query("select * from employee_leave where employeeId=".$id." and leaveId=5 and '".$dyt."' between startDate and endDate"));
	if($eleave->ndex){
		return 0; 
	}
	else{
...
function checkDay2($dyt,$id){

	$eleave=mysql_fetch_object(mysql_query("select * from employee_leave where employeeId=".$id." and leaveId=13 and '".$dyt."' between startDate and endDate"));
	if($eleave->ndex){
		return 0; 
	}
	else{
	...
	while($r=mysql_fetch_object($exec)){
		 ...
		 $totalabs=0;
			for ( $i = $start; $i <= $end; $i += 86400 ){
				...
				if($_POST['absent_type']=='1'){
					if(checkDay1($datelog,$r->emp)==0){
						$totalabs++;
						$ctr1s++;
				...
				elseif($_POST['absent_type']=='2'){
					if(checkDay2($datelog,$r->emp)==0){
						$totalabs++;
						$ctr1s++;
		  			    if ($ctr1s==2){$bgclr1s='#ffffff';$ctr1s=0;} else { $bgclr1s='#F8F8AC';}
						$latedata.="".$datelog.",";
				...
				elseif($_POST['absent_type']=='3'){
					if(checkDay3($datelog,$r->emp)==0){
						$totalabs++;
						$ctr1s++;
		  			    if ($ctr1s==2){$bgclr1s='#ffffff';$ctr1s=0;} else { $bgclr1s='#F8F8AC';}
						$latedata.="".$datelog.",";
