...
if ($_GET['pageact'] == "runCutoff"){
	...
	while ($dtupd =  mysql_fetch_assoc($rsUpd)){
		...
		} elseif ($dtupd['leaveId'] == '17'){
			$update = mysql_query("UPDATE dailytimesummary SET spl_lve='1', days_work='1' WHERE ndex='".$dtupd['ndex']."'",$conn);

		} elseif ($dtupd['leaveId'] == '4'){
			$update = mysql_query("UPDATE dailytimesummary SET ml_lve='1', days_work='1' WHERE ndex='".$dtupd['ndex']."'",$conn);

		} elseif ($dtupd['leaveId'] == '16'){
			$update = mysql_query("UPDATE dailytimesummary SET mcl_lve='1', days_work='1' WHERE ndex='".$dtupd['ndex']."'",$conn);
		...
		while ($dt = mysql_fetch_assoc($rs)){
			...
			$updatePayroll = mysql_query("UPDATE payroll SET 
							 ...
							 , payrollRemarks = '".$payrollRem."'
									WHERE empid='".$dt['employeeId']."' 
									&& pay_period='".$getInfoCutoffDate['payrollDate']."'",$conn);
