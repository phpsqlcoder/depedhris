...
$sql = "SELECT e.lastName , e.firstName, m.* FROM mortuary  m
									LEFT JOIN employee e ON e.ndex=m.employeeId 
											WHERE 1";
if ($_GET['pageact'] == 'add'){
	$result = mysql_query("INSERT INTO mortuary ( `employeeId` , `amount` , `payrollDate`, `remarks`)
					VALUES ('".$_POST['employeeId']."', '".$_POST['amount']."', '".$_POST['payrollDate']."', '".$_POST['remarks']."')",$conn);
	header ("Location: ".$_SERVER['PHP_SELF']."");
} elseif ($_GET['pageact'] == "edit"){
	$result = mysql_query("UPDATE mortuary SET employeeId='".$_POST['employeeId']."',
																						 amount='".$_POST['amount']."',
	...
	$result = mysql_query("DELETE FROM mortuary WHERE ndex = '".$_GET['id']."'",$conn);
	header ("Location: ".$_SERVER['PHP_SELF']."");
} elseif ($_GET['pageact'] == "search" && !empty($search_text)){
...
$sql .= " ORDER BY m.payrollDate DESC";
$showPerPage = 50;
$nPageDisplay = 5;
...
if ($myact == 'add'){
	for ($i=1;$i<10;$i++){
		...
		$rowCount = mysql_num_rows(mysql_query("SELECT * FROM mortuary WHERE payrollDate >= '".$nextPayrollDate."' ORDER BY payrollDate DESC LIMIT 1",$conn));
	...
	$infoMortuary  =  mysql_fetch_assoc(mysql_query("SELECT * FROM dedsetup WHERE dedType='MORTUARY'",$conn));
	$rs1['amount'] = $infoMortuary['amount'];
}
	 	...
	 	<table cellpadding="0" cellspacing="0" border="0" width="1020">
			<td  width="415" valign="top">
				...
				<table cellpadding="3" cellspacing="0" border="0">
					<tr><td colspan="2" align="right"><br /><b>::: 	</b><i>Add Mortuary</i><hr></td></tr>
			...
			<td valign="top">
				<table cellpadding="0" cellspacing="0" border="0" width="90%">
					<tr><td colspan="2" align="right"><br /><b>:::</b><i> List of Mortuary Created</i><hr></td></tr>
					<tr bgcolor="#e6e0df">
