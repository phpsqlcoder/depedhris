...
if ($_GET['pageact'] == "runComputation"){
	...
	// END FREEZE DEDUCTION CONDITION
		...
		$thirthyPercentOfGross = ($grossPay * 0.40);
		...
		if ($thirthyPercentOfGross > ($netPay)){
			if ($r['d_coopTotal']>0){
				...
				$coopDedCurPayroll = $r['d_coopTotal'] - (($grossPay * 0.40) - $netPay);
		...
		if ($thirthyPercentOfGross > ($netPay)){
			if ($r['financialAssistance']>0){
				//echo $r['d_coopTotal']." > 0";
				$fassistanceDedCurPayroll = $r['financialAssistance'] - (($grossPay * 0.40) - $netPay);
		...
		if ($thirthyPercentOfGross > ($netPay)){
			if ($r['d_mortuary']>0){
				//echo $r['d_coopTotal']." > 0";
				$mortuaryDedCurPayroll = $r['d_mortuary'] - (($grossPay * 0.40) - $netPay);
		...
		if ($thirthyPercentOfGross > ($netPay)){
			if ($r['d_other']>0){
				//echo $r['d_coopTotal']." > 0";
				$dotherDedCurPayroll = $r['d_other'] - (($grossPay * 0.40) - $netPay);
