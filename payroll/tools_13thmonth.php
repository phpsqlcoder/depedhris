	...
	foreach($_POST['empNo'] AS $empNoS){
		...
		$update = mysql_query("UPDATE payroll13thmonth SET wtax = '".$_POST['wtax'.$empNoS]."', 
														   ...
														   cashAdvance = '".$_POST['cashAdvance'.$empNoS]."'
																WHERE empNo='".$empNoS."' && cutOffDate='".$rsCutoff['cutOffDate']."' ", $conn);
	}
	header("Location: ?");
}
...
while ($r =  mysql_fetch_assoc($rs)){
	...
	$data .= "<input type='hidden' name='empNo[]' value='".$r['empNo']."'>
			...
			<td><input type='Text' value='".$r['cashAdvance']."' name='cashAdvance".$r['empNo']."' style='text-align:right;'></td>
			<td><input type='Text' value='".$r['otherDeduction']."' name='otherDeduction".$r['empNo']."' style='text-align:right;'></td></tr>";
}
