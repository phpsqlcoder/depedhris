...
if ($_GET['pageact'] == "runComputation"){
	...
	$sql = "SELECT e.ndex employeeId, e.isUnionMember, e.isCoopMember, e.paytype, e.isTaxable,
								p.*, e.employmentStatus, e.residencyTrainingProgram, e.level,
								 ec.allowance, ec.honorarium, ec.cola, ec.basicPay basicPay, ec.taxType, ec.payTypeNdex, ec.pagibigSavings
										FROM employee e 
	...
	// END FREEZE DEDUCTION CONDITION
		...
		//echo "[ ".$dt['d_hospital']." ]";
		//die(); UPDATE  payroll SET d_other=0, d_sssloan=0, pagibigloan=0, d_hospital=0, pagibigloanh=0 where pay_period='".$_POST['cutoffDate']."'
