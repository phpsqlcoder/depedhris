...
if ($_GET['pageact'] == "runComputation"){
	...
	$tables ='<table width="250%" border="1">';
	$tables .='<tr>
						<td>Name</td>
						<td>Gross</td>
						<td>Deduction</td>
	...
	while($dt = mysql_fetch_assoc($rs)){
		...
		if($checkIfOnFreeze==0){ //ADDED BY JUNDRIE - TO DEDUCT ONLY THOSE EMPLOYEES WHO ARE MARK AS FREEZE DEDUCTION. DATA is at table loanpayments_freeze
			...
			$hosp_total = mysql_fetch_array(mysql_query("select sum(Amount) as balance from ar_hospital_ee_trx where employeeId='".$dt['employeeId']."'"));
			$hosp_paid = mysql_fetch_array(mysql_query("select sum(amountPaid) as paid from ar_hospital_ee_payment_ledger where employeeId='".$dt['employeeId']."' and status='POSTED'"));
			$hosp_refund = mysql_fetch_array(mysql_query("select sum(amountPaid) as refund, sum(payment) as payment_refund from ar_hospital_ee_refund_ledger where employeeId='".$dt['employeeId']."' and status='POSTED'"));
			$hosp_bal = ($hosp_total['balance'] + $hosp_refund['refund']) - ($hosp_paid['paid'] + $hosp_refund['payment_refund']);
			$d_hospital = 0;
			if($hosp_bal > 0){
				$hosp_monthly_pay = mysql_fetch_array(mysql_query("select amortization from hospital_deduction_amortization where employeeId='".$dt['employeeId']."' order by id desc limit 1"));
				$d_hospital = $hosp_monthly_pay['amortization'];
			}
		}
		...
		//die();
		/*
		$updatePayrolltable = mysql_query("
			UPDATE payroll SET
				cola='".$cola."',
		...
		//echo $dt['employeeId']."<br />";
			*/
		$grossPay = $netBasic + $cola + $allowance + $honorarium + $night_prem + $ot_reg + $ot_exc + $otherIncome + $dt['onCallOvertime'] + $duty_rd + $spholiday + $lholiday + $otRDLHoliday + $otRDSHoliday + $otLHoliday + $otSHoliday + $otRestDay - $undertime +  $dt['adj_other'] +  $hazardPay +  $incentive;
		$totalDeduction = $dt['d_pnb'] + $dt['d_parkingFee'] + $taxwithHeld + $sssEmployeeShare + $philhealthDed + $pagibigDed + $dt['pagibigloan'] + $dt['pagibigloanh'] + $dt['d_unionDues'] + $mortuaryDed['amount'] + $dt['d_sssloan'] + $d_hospital + $dt['d_cashAdvance'] + $dt['d_other'] + $d_other + $dt['d_coopTotal'] + $faDed['amount'] + $dt['pagibigSavings'];
		...
		$eee = mysql_fetch_assoc(mysql_query("SELECT * FROM employee WHERE ndex='".$dt['employeeId']."'",$conn));
		//echo $eee['lastName'].', '.$eee['firstName'].' '.$eee['middleName'].'<br>';
		if ($thirthyPercentOfGross > ($netPay)){
			$total_emp++;
			$tables .='<tr>
						<td>'.$eee['lastName'].', '.$eee['firstName'].' '.$eee['middleName'].'</td>
						...
						<td>'.number_format($sssEmployeeShare,2).'</td>
						<td>'.number_format($philhealthDed,2).'</td>
						<td>'.number_format($taxwithHeld,2).'</td>
						<td>'.number_format($pagibigDed,2).'</td>
						<td>'.number_format($dt['d_unionDues'],2).'</td>
						<td>'.number_format($dt['d_sssloan'],2).'</td>
						<td>'.number_format($d_hospital,2).'</td>
						<td>'.number_format($dt['pagibigloanh'],2).'</td>
						<td>'.number_format($dt['pagibigloan'],2).'</td>
						<td>'.number_format($dt['pagibigSavings'],2).'</td>
						<td>'.number_format(($d_other + $r['d_other']),2).'</td>
			</tr>';
		}
...
<div id="main_content_wrap" class="container_12">
	...
	if ($_GET['pageact'] == "runComputation"){
		if($total_emp>1){
			echo '<p>Please freeze the deductions of the following employee:</p>';
		}
		else{
			echo '<p>No Record Found. You may now proceed to Payroll Calculation. Click <a href="tools_govpremium.php">here</a></p>';
		}
	}
	?>
	<?php echo $tables; ?>
