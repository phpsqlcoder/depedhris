...
if ($_GET['pageact'] == "updatePayroll"){
	...
	for($i=1;$i <= $_POST['numrows']; $i++){
		...
		//echo $_POST['rowNum'.$i]."/".$_POST['holdPay'.$i]."<br>";
		if ($_POST['holdPay'.$i] == 'on'){
			$updateCoopPayroll = mysql_query("UPDATE payroll SET holdSalary='1' WHERE ndex='".$_POST['rowNum'.$i]."'",$conn);
		} else {
			$updateCoopPayroll = mysql_query("UPDATE payroll SET holdSalary='' WHERE ndex='".$_POST['rowNum'.$i]."'",$conn);
		}
		
	}
} // end

$sql = "SELECT p.*, e.lastName, e.firstName,e.employmentStatus,e.employeeNo FROM payroll p 
...
while ($dt = mysql_fetch_assoc($rs)){
	...
	$checkHoldSalary = '';
	if ($dt['holdSalary'] == '1'){ $checkHoldSalary = " checked='checked'";}
	$rowRes .= "<tr  style='background-color:".$tr_bcg.";margin:8px 8px;'><td style='padding:5px;'>".getID($dt['employmentStatus'],$dt['employeeNo'])." - ".$dt['lastName'].", ".$dt['firstName']."</td><td><input type='hidden' name='rowNum".$cnt."' value='".$dt['ndex']."'><input type='checkbox' name='holdPay".$cnt."' ".$checkHoldSalary."></td></tr>";
}

//-------------------------------------------------
//-------------------------------------------------
$rs = mysql_query("SELECT * FROM cutoffdates WHERE 1 ORDER BY payrollDate DESC limit 2",$conn);
while ($dt = mysql_fetch_assoc($rs)){
...
<?php include "header.php";?>
    ...
    <div class="clearfix"> 
			...
			<?php if ($countRes != 0){ echo "<br><br>PAYROLL PERIOD: ".date('F d, Y',strtotime($_POST['cutoffDate']));?>
			...
			<form method="POST" action="<?php echo $_SERVER['PHP_SELF'];?>?pageact=updatePayroll" margin="0px;" name="myForm">
				...
				<table cellapdding="10" cellspacing="10" ">
					<tr class="columnheader" bgcolor="000000"><td style="padding:10px;">NAME</td><td>HOLD?</td></tr>
					<?php echo $rowRes;?>
				</table>
