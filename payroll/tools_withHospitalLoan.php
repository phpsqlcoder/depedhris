...
$qry = mysql_query("select distinct e.ndex as endex,e.firstName,e.lastName,e.employmentStatus,e.employeeNo,e.middleName,d.name as deptname,count(h.ndex) as total_trx from ar_hospital_ee_trx h left join employee e on e.ndex=h.employeeId
	left join dept d on d.ndex=e.deptId where e.ndex>0 ".$q."
	group by e.ndex,e.firstName,e.lastName,e.middleName,d.name,e.employmentStatus,e.employeeNo
	order by e.lastName,e.firstName,e.middleName");

while($r = mysql_fetch_array($qry)){
	//$total['balance'] = 0;
	//echo "select sum(Amount) as balance from ar_hospital_ee_trx where employeeId='".$r['endex']."'";

	$total = mysql_fetch_array(mysql_query("select sum(Amount) as balance from ar_hospital_ee_trx where employeeId='".$r['endex']."'"));
	$paid = mysql_fetch_array(mysql_query("select sum(amountPaid) as paid from ar_hospital_ee_payment_ledger where employeeId='".$r['endex']."' and status='POSTED'"));
	$refund = mysql_fetch_array(mysql_query("select sum(amountPaid) as refund, sum(payment) as payment_refund from ar_hospital_ee_refund_ledger where employeeId='".$r['endex']."' and status='POSTED'"));
	$ctr1s++;
	...
	$data.='<tr style="background-color:'.$bgclr1s.'">
				...
				<td align="right">'.number_format($refund['refund'],2).'</td>
				<td align="right">'.number_format($paid['paid'],2).'</td>
				<td align="right">'.number_format((($total['balance'] + $refund['refund']) - ($paid['paid'] + $refund['payment_refund'])),2).'</td>
				<td align="center">'.$r['total_trx'].'</td>
				<td align="right"><a href="tools_withHospitalLoan_individual.php?id='.$r['endex'].'">View</a></td>
...
<div id="main_content_wrap" class="container_12" style="width:80%">
	<div id="row">
		<div class="col-md-6">
    ...
    <table width="100%">
			<thead>
				<tr>
					...
					<th align="right" style="width:8%">Refund</th>
