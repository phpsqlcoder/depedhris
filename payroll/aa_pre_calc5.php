...
if ($_GET['pageact'] == "runComputation"){
	...
	// END FREEZE DEDUCTION CONDITION
		...
		//$totalDeduction = $r['d_pnb'] + $r['d_parkingFee'] + $taxwithHeld + $sssEmployeeShare + $philhealthDed + $pagibigDed + $dt['pagibigloan'] + $dt['pagibigloanh'] + $dt['d_unionDues'] + $mortuaryDed['amount'] + $dt['d_sssloan'] + $d_hospital + $r['d_cashAdvance'] + $d_other + $faDed['amount'] + $dt['pagibigSavings'];
		$totalDeduction = $r['d_pnb'] + $r['d_parkingFee'] + $taxwithHeld + $sssEmployeeShare + $philhealthDed + $pagibigDed + $dt['pagibigloan'] + $dt['pagibigloanh'] + $dt['d_unionDues'] + $dt['d_sssloan'] + $d_hospital + $r['d_cashAdvance'] + $dt['pagibigSavings'];
		...
		if ($thirthyPercentOfGross > ($netPay)){
			...
			$data.="<tr bgcolor='".$bgclr1s."' align='right'>
				...
				<td>".number_format($faDed['amount'],2)."&nbsp; </td>

				<td>".number_format($mortuaryDed['amount'] ,2)."&nbsp; </td>
				<td>".number_format($d_other,2)."</td>
