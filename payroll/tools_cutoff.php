...
if ($_GET['pageact'] == "runCutoff"){
	...
	while ($dtupd =  mysql_fetch_assoc($rsUpd)){
		...
		if($dtupd['holiday'] == 'S'){
			$update = mysql_query("UPDATE dailytimesummary SET spholiday='".$dtupd['hoursDuty']."' WHERE ndex='".$dtupd['ndex']."'",$conn);
		} elseif($dtupd['holiday'] == 'L') {
			$update = mysql_query("UPDATE dailytimesummary SET lholiday='".$dtupd['hoursDuty']."' WHERE ndex='".$dtupd['ndex']."'",$conn);
		}
		
		if($dtupd['isDayOff'] == '1'){
			$update = mysql_query("UPDATE dailytimesummary SET duty_rd='".$dtupd['hoursDuty']."' WHERE ndex='".$dtupd['ndex']."'",$conn);
		}
	...
	/*if (isErrorTimeSummary($getInfoCutoffDate['cutoffDateStart'],$getInfoCutoffDate['cutoffDateEnd']) == 1){
		...
		$sql = "SELECT e.ndex as employeeId, SUM(dts.days_absent) daysAbsent, SUM(dts.minutesLate + dts.undertime) undertime, SUM(dts.approvedOvertime) otReg, SUM(dts.ot_exc) otExc, SUM(dts.days_work) daysWork
