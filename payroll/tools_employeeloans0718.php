...
$sql = "SELECT l.*, e.lastName, e.firstName, e.ndex employeeNdex FROM loansetup  l 
										LEFT JOIN employee e ON e.ndex=l.employeeId
											WHERE 1 && l.isDeleted<>'1'";
if ($_GET['pageact'] == 'add'){

	$result = mysql_query("INSERT INTO loansetup ( `employeeId` , `loanType` , `loanAmount` , `nOfDeduction` , `dedDateStart` , `dedAmount` , `remarks` , `loanBalance` )
					VALUES ('".$_POST['employeeId']."', '".$_POST['loanType']."', '".$_POST['loanAmount']."', '".$_POST['nOfDeduction']."', '".$_POST['dedDateStart']."', '".($_POST['loanAmount'] / ($_POST['nOfDeduction'] * 2))."', '".$_POST['remarks']."', '".$_POST['loanAmount']."' )",$conn);
	header ("Location: ".$_SERVER['PHP_SELF']."");
} elseif ($_GET['pageact'] == "edit"){
	$result = mysql_query("UPDATE loansetup SET employeeId='".$_POST['employeeId']."', loanAmount='".$_POST['loanAmount']."',
						    loanType='".$_POST['loanType']."',
						    nOfDeduction='".$_POST['nOfDeduction']."',
						dedDateStart='".$_POST['dedDateStart']."',
						dedAmount='".($_POST['loanAmount'] / ($_POST['nOfDeduction'] * 2))."',
						remarks='".$_POST['remarks']."',
						postedDate='".date("Y-m-d H:i:s")."',
						loanBalance='".$_POST['loanAmount']."' WHERE ndex='".$_POST['id']."'",$conn);
	$linkback = explode('?',$_SERVER['PHP_SELF']);
	header ("Location: ".$linkback[0]."");
} elseif ($_GET['pageact'] == "delete"){
	$result = mysql_query("UPDATE loansetup SET `isDeleted`='1' WHERE ndex = '".$_GET['id']."'",$conn);
	$linkback = explode('?',$_SERVER['PHP_SELF']);
	header ("Location: ".$linkback[0]."");
	//header ("Location: ".$_SERVER['PHP_SELF']."");
...
if (empty($_GET['myact']) && empty($_GET['pageact'])){
	...
	//die();
	$postLoan = mysql_query("UPDATE loansetup SET posted='1', postedDate='".date('Y-m-d H:i:s')."' where ndex = ".$_GET['id']."",$conn);
...
while ($dt = mysql_fetch_assoc($rs)){
	...
	$viewSystemDepartmentList .= "<tr class='".$tr_bcg."' style='font-size:12px;background-color:".$bgclr1s.";color:black;'>
			<td><a href='tools_employeeloanslledger.php?id=".$dt['employeeNdex']."'>".$dt['lastName'].", ".$dt['firstName']." </a></td>
			...
			<td align='right'>".number_format($dt['loanBalance'],2)."</td>
...
$loanType = array('SSS'=>'SSS LOAN','PAG-IBIG'=>'PAG-IBIG HOUSING LOAN','PAG-IBIGSAL'=>'PAG-IBIG SALARY LOAN','CASHADVANCE'=>'CASH ADVANCE','HOSPITAL'=>'HOSPITAL','CALAMITYLOAN'=>'CALAMITY LOAN','OTHERS'=>'OTHERS');
foreach($loanType as $loanTypeId => $loanTypeName){
	$optionSelectloanType .= "<option value='".$loanTypeId."'>".$loanTypeName."</option>";
	if ($loanTypeId == $rs1['loanType']){
		$optionSelectloanType_selected = "<option value='".$loanTypeId."'>".$loanTypeName;
	}
}
	 	...
	 	<table>
			...
			<td  width="415" valign="top">
				...
				<table cellpadding="3" cellspacing="0" border="0">
					...
					<tr><td>Loan Type</td><td><select name="loanType"><?php echo $optionSelectloanType_selected.$optionSelectloanType;?></select></td></tr>
					<tr><td>Loan Amount</td><td><input type="number" name="loanAmount" id="loanAmount" value="<?php echo $rs1['loanAmount']?>"></td></tr>
					<tr><td>No. of Deduction</td><td><input type="text" name="nOfDeduction" value="<?php echo $rs1['nOfDeduction']?>"></td></tr>
					...
					<tr>
						<td colspan="2" align="left">
							...
							<table cellpadding="5" cellspacing="0" border="0" width="100%">
								<tr class="columnheader"><td>Name</td><td align="right">LoanAmount</td><td align="right">No. of ded</td><td  align="right">Start of Ded<br />(Date)</td><td  align="center">Balance</td><td align="right">Action</td></tr>
