...
$objPHPExcel->getProperties()->setCreator("HRIS System")
	...
	$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('B1', $titled)
			;
	$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A2', 'ID')
            ->setCellValue('B2', 'Name')
            ->setCellValue('C2', "Department")
			->setCellValue('D2', "Deduction")
			;
	$seq=2;
	$total=0;
	$genqry="select  e.*,d.name as dep from employee e left join dept d on d.ndex=e.deptId where e.isActive=1 and e.isCoopMember=1";
	...
	while($r=mysql_fetch_object($qry)){		
			...
			->setCellValue("C$seq", $r->dep)
			->setCellValue("D$seq", '0.00');
			//$objPHPExcel->getActiveSheet()->getStyle('D$seq')->getNumberFormat();
	}
$objPHPExcel->getActiveSheet()->setTitle('Coop');
