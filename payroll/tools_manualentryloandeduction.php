...
if ($_GET['pageact'] == "updatePayroll"){
	...
	for($i=1;$i <= $_POST['numrows']; $i++){
		...
		$updateCoopPayroll = mysql_query("UPDATE payroll SET d_sssloan='".$_POST['d_sssloan'.$i]."', pagibigloan='".$_POST['pagibigloan'.$i]."', pagibigloanh='".$_POST['pagibigloanh'.$i]."' WHERE ndex='".$_POST['rowNum'.$i]."' && pay_period='".$_POST['cutoffDate']."'",$conn);
...
while ($dt = mysql_fetch_assoc($rs)){
	...
	$rowRes .= "<tr  style='background-color:".$tr_bcg.";margin:8px 8px;'>
								...
								<td><input type='hidden' name='rowNum".$cnt."' value='".$dt['ndex']."'><input type='text' name='d_sssloan".$cnt."' value='".$dt['d_sssloan']."'></td>
								<td><input type='hidden' name='rowNum".$cnt."' value='".$dt['ndex']."'><input type='text' name='pagibigloan".$cnt."' value='".$dt['pagibigloan']."'></td>
								<td><input type='hidden' name='rowNum".$cnt."' value='".$dt['ndex']."'><input type='text' name='pagibigloanh".$cnt."' value='".$dt['pagibigloanh']."'></td>
							</tr>";
}
    ...
    <div class="clearfix"> 
			...
			<form method="POST" action="<?php echo $_SERVER['PHP_SELF'];?>?pageact=updatePayroll" margin="0px;" name="myForm">
				...
				<table cellapdding="10" cellspacing="10" ">
					<tr class="columnheader" bgcolor="000000"><td style="padding:10px;">NAME</td><td>SSS LOAN</td><td>HDMF <br>SALARY LOAN</td><td>HDMF <br>HOUSING LOAN</td></tr>
					<?php echo $rowRes;?>
				</table>
