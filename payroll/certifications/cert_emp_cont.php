...
$pageTitle='SSS Employee Contribution';
?>
...
$sssno="SSS NO. ".$emp->sssNumber;
$pq=mysql_query("select * from payroll where pay_period>='".getpayperiod($_POST['st'])."' and pay_period<='".getpayperiod($_POST['en'])."' and empid='".$_POST['id']."' and days_work>0 and d_sss>0");

while($p=mysql_fetch_object($pq)){
	$table='tbl_sss';
	if($p->pay_period<='2013-12-31'){$table='tbl_sss01112014';}
	$c_sss=mysql_fetch_object(mysql_query("select * from ".$table." where e_share=".$p->d_sss.""));
	...
	$data.="<tr style='background-color:".$bgclr1s.";'>
				<td>".$p->pay_period."</td>
				<td>".number_format($p->d_sss,2)."</td>
				<td>".number_format($c_sss->c_share,2)."</td>
				<td>".$or[$or_field]."</td>
...
<table style="font-family:Arial;font-size:16px;" width="100%">
	...
	<tr><td colspan="3">&nbsp;</td><td colspan="4">DAVAO DOCTORS HOSPITAL<br>SSS No. <?php echo $_POST['h5']?><br><br><br></td></tr>
	<tr><td colspan="3">&nbsp;</td><td colspan="4">By:<br><?php echo $_POST['h6']?><br>COMPENSATION & BENEFIT HEAD<br></td></tr>
</table>
<?php
}
	else 
...
<form name="frmcompo" action="cert_emp_cont.php?act=go" method="post">
<table style="font-family:Arial;font-size:12px;">
	...
	<tr><td>Top Paragraph:</td><td><textarea name="h2" rows="3" cols="60">Herewith are the premium payments of <employee> with <sss no> follows:</textarea></td></tr>
	...
	<tr><td>DDH SSS:</td><td><textarea name="h5" rows="3" cols="60">09-0448000-1</textarea></td></tr>
	<tr><td>SIGNATORY:</td><td><textarea name="h6" rows="3" cols="60">MARIA DOLORES B. BENIGNO</textarea></td></tr>
	<tr><td>POSITION:</td><td><textarea name="h7" rows="3" cols="60">BENEFITS OFFICER</textarea></td></tr>
