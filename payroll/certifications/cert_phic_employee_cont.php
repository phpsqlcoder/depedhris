...
$pageTitle='PHIC Employee Contribution';
?>
...
$pc=mysql_fetch_object(mysql_query("select * from employee_compensation where employeeId=".$emp->ndex.""));
$basicpay=$p->basicPay + $p->cola + $p->honorarium;
$employeename="<strong>".$emp->lastName.",".$emp->firstName." ".$emp->middleName.",</strong>";
$phicNumber="PEN NO. ".$emp->phicNumber;
$pq=mysql_query("select * from payroll where pay_period>='".getpayperiod($_POST['st'])."' and pay_period<='".getpayperiod($_POST['en'])."' and empid='".$_POST['id']."' and days_work>0 and pagibig>0");


while($p=mysql_fetch_object($pq)){
	$ephic=philHelthPremium($p->basicpay);
	...
	$data.="<tr style='background-color:".$bgclr1s.";'>
				<td>".$p->pay_period."</td>
				<td>".number_format($ephic['eeShare'],2)."</td>
				<td>".number_format($ephic['companyShare'],2)."</td>
				<td>".$or[$or_field]."</td>
...
<table style="font-family:Arial;font-size:16px;" width="100%">
	...
	<tr><td colspan="6" align="center"><u><strong style="font-size:20px;">CERTIFICATE OF PREMIUM PAYMENTS</strong></u><br><br></td></tr>
	<tr><td colspan="6"><?php echo $_POST['h1']?><br><br></td></tr>
	<tr><td colspan="6"><p><?php echo str_replace("<pen no>",$phicNumber,str_replace("<employee>",$employeename,$_POST['h2']));?></p><br></td></tr>
	...
	<tr><td colspan="3">&nbsp;</td><td colspan="4">By:<br><?php echo $_POST['h6']?><br>BENEFITS OFFICER<br></td></tr>
</table>
<?php
}
	else 
...
<form name="frmcompo" action="cert_phic_employee_cont.php?act=go" method="post">
<table style="font-family:Arial;font-size:12px;">
	...
	<tr><td>Top Paragraph:</td><td><textarea name="h2" rows="3" cols="60">Herewith are the premium payments of <employee> with <pen no> follows:</textarea></td></tr>
	<tr><td>Bottom Paragraph:</td><td><textarea name="h3" rows="3" cols="60">This certification is issued upon the request of the above-named for Premium Payments purposes only.</textarea></td></tr>
	...
	<tr><td>DDH PEN:</td><td><textarea name="h5" rows="3" cols="60">23-02240002-01</textarea></td></tr>
	<tr><td>SIGNATORY:</td><td><textarea name="h6" rows="3" cols="60">MARIA DOLORES B. BENIGNO</textarea></td></tr>
	<tr><td>POSITION:</td><td><textarea name="h7" rows="3" cols="60">HR AND ADMIN DIRECTOR</textarea></td></tr>
