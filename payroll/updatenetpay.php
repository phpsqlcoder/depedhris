...
$cutoffDate = '2014-09-30';
die();
//------------------------------------

$sql = "SELECT * FROM payroll WHERE pay_period='".$cutoffDate."'";// and empid=882";
...
while ($dt = mysql_fetch_assoc($rs)){
		...
		if ($thirthyPercentOfGross > ($netPay)){
			...
			if ($r['d_coopTotal']>0){
				...
				$updpayroll = mysql_query("UPDATE payroll SET d_coopTotal='".$coopDedCurPayroll."' WHERE ndex='".$dt['ndex']."' && pay_period='".$cutoffDate."'",$conn);
		...
		if ($thirthyPercentOfGross > ($netPay)){
			...
			if ($r['d_hospital'] > 0){
				...
				//echo "$thirthyPercentOfGross > ($netPay) <br> ".$r['d_hospital']." ppp $hospitalDedCurPayroll";
				//echo "UPDATE payroll SET d_hospital='".$hospitalDedCurPayroll."' WHERE ndex='".$dt['ndex']."' && pay_period='".$_POST['cutoffDate']."'";
				//die();
				$updpayroll = mysql_query("UPDATE payroll SET d_hospital='".$hospitalDedCurPayroll."' WHERE ndex='".$dt['ndex']."' && pay_period='".$cutoffDate."'",$conn);
