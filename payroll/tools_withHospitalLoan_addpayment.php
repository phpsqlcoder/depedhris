...
if(isset($_GET['submit'])){
		...
		$bills =  mysql_query("select * from ar_hospital_ee_trx 
			where employeeId='".$l['endex']."' and AR_No = '".$l['AR_No']."' and Batch_No='".$l['Batch_No']."' and Status='Active' 
and ndex='".$_GET['id']."' order by priorityNo");
		...
		while($bill = mysql_fetch_array($bills)){
			...
			if($running_balance > 0){
				...
				$insert_payment = mysql_query("INSERT INTO `ar_hospital_ee_payment_ledger`(`ar_hospital_ee_trx_Id`, `Batch_No`, `AR_No`, `employeeId`, `paymentDate`, `amountPaid`, `lineBalance`, `paymentType`, `remarks`, `isUploaded`,`or_number`,`payment_type`) 
					VALUES ('".$bill['ndex']."', '".$bill['Batch_No']."', '".$bill['AR_No']."', '".$bill['employeeId']."', '".$_POST['paymentDate']."',
 '".$amount_to_pay."', '".$line_balance."', 'Manual', '".$_POST['remarks']."','', '".$_POST['or']."', '".$_POST['payment_type']."')");
		...
		if($running_balance > 0){
			$refunds_qry =  mysql_query("select * from ar_hospital_ee_refund_ledger 
			where employeeId='".$l['endex']."' and AR_No = '".$l['AR_No']."' and Batch_No='".$l['Batch_No']."' and payment<=amountPaid and ar_hospital_ee_trx_Id='".$_GET['id']."'  order by ndex desc");
			while($ref = mysql_fetch_array($refunds_qry)){
				if($running_balance > 0){
					$balance_per_refund = $ref['amountPaid'] - $r['payment'];
					if($balance_per_refund > $running_balance){
						$amount_to_pay_refund = $running_balance;
						$running_balance = 0;						
					}
					else{
						$amount_to_pay_refund = $balance_per_refund;
						$running_balance = $running_balance - $balance_per_refund;
					}
					
					$pay_refund = mysql_query("update ar_hospital_ee_refund_ledger set payment = '".$amount_to_pay_refund."' where ndex='".$ref['ndex']."'");
				}
			}
		}
		header("location:tools_withHospitalLoan_individual.php?id=".$l['endex']."&success=go");
...
<div id="main_content_wrap" class="container_12">
    ...
    <table width="50%" cellspacing="10" cellpadding="10">
		...
		<tr>
			<td>Amount:</td>
			<td><input type="number" min="0.01" required="required" step="0.01" name="amountPaid" value="0.00"><br>
