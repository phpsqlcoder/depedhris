...
$sql = "SELECT e.lastName, e.firstName, e.middleName, e.employmentStatus, e.employeeNo, d.name departmentName, p.empid, p.*
								FROM employee e 
									...
									LEFT JOIN dept d ON d.ndex = e.deptId
										WHERE p.pay_period='".$_POST['PayrollCutoff']."' && p.residencyTrainingProgram='' && p.basicpay<>0 && p.holdSalary<>'1'";// && e.ndex=163";
...
while($r=mysql_fetch_assoc($exec)){
		/*$grossPay = $r['netBasic'] + $r['cola'] + $r['allowance'] + $r['honorarium'] + $r['payNightPremium'] + $r['payOTReg'] + $r['payOTExc'] + $r['oth_income'] + $r['onCallOvertime'] + $r['payDutyRd'] + $r['paySpHoliday'] + $r['payLHoliday'] - $r['payUndertime'] +  $r['adj_other'] + $r['otRDLHolidayPay'] + $r['otRDSHolidayPay'] + $r['otLHolidayPay'] + $r['otSHolidayPay'] + $r['otRestDayPay'] + $r['adj_13th_mon_pay'];*/
...
if($_POST['eksel']=='on'){
	...
	?>
	 ...
	 <thead>
	  <tr>
	       <td colspan="24" align="center" style="font-size:13px;">DAVAO DOCTORS HOSPITAL<br>PAYROLL REPORTS<br /><?php echo $reportTitle;?><br>
