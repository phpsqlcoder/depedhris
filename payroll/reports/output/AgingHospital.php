...
if ($_POST['startdate']){
	$sql .= " && p.trxDate >= '".$_POST['startdate']."' and p.trxDate<='".$_POST['enddate']."'";
}
	...
	if($r['Amount'] > 0){
	    ...
	    $data.="<tr bgcolor='".$bgclr1s."' align='right'>
						      ...
						      <td>".$r['Batch_No']."</td>
							   	...
							   	$st = $month = strtotime($_POST['startdate']);
				      			$en = strtotime($_POST['enddate']);
				      			while($month < $en){
				      				
				      				$paids = mysql_fetch_array(mysql_query("select sum(amountPaid) as totald from ar_hospital_ee_payment_ledger where employeeId='".$r['endex']."' and paymentDate>='".date('Y-m-1',$month)."' and paymentDate<='".date('Y-m-t',$month)."' and ar_hospital_ee_trx_Id='".$r['hnd']."'"));
				      				//echo "select sum(amountPaid) as total from ar_hospital_ee_payment_ledger where employeeId='".$r['endex']."' and paymentDate>='".date('Y-m-1',$month)."' and paymentDate<='".date('Y-m-t',$month)."'<br>";
				      				$data.="<td>".number_format($paids['totald'],2)."</td>";
