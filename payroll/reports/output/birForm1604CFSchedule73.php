...
$sql = "SELECT e.ndex empid, e.lastName, e.firstName, e.middleName, e.employeeNo, e.tin, d.name, ec.basicPay, e.civilStatus, ec.payTypeNdex
								...
								LEFT JOIN payroll p ON p.empid=e.ndex
										WHERE DATE_FORMAT(p.pay_period,'%Y-12') = '".$_POST['year']."-12' 
											  AND e.residencyTrainingProgram<>'ROD'";// && e.ndex=828";
...
while($r=mysql_fetch_assoc($exec)){
	...
	*/
	$rDec = mysql_fetch_assoc(mysql_query("SELECT SUM( d_whtax  ) AS decTaxDue FROM `payroll` WHERE DATE_FORMAT( `pay_period` , '%Y-%m' ) = '".$_POST['year']."-12' && `empid` = '".$r['empid']."'"));

	$rPayroll = mysql_fetch_assoc(mysql_query("SELECT basicpay AS basicPay, pay_type FROM `payroll` WHERE pay_period = '".$_POST['year']."-12-31' && `empid` = '".$r['empid']."'"));
	...
	//echo UnusedLeave($r['empid'],$_POST['year'])." --- ".$dailyRate." rp ".$rPayroll['basicPay']." r ".$r['basicPay'];
	 ...
	 <thead>
	  <tr>
	    <td align="LEFT" style="font-size:13px;" colspan="10">
		   BIR FORM 1604CF - SCHEDULE 7.3<br>
