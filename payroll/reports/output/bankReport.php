...
$sql = "SELECT e.lastName, e.firstName, e.middleName, e.employmentStatus, e.employeeNo, d.name departmentName, e.bankAccountNo, p.*
								FROM employee e 
									...
									LEFT JOIN dept d ON d.ndex = e.deptId
										WHERE e.isActive='1' && p.pay_period='".$_POST['PayrollCutoff']."' && e.residencyTrainingProgram='' && p.holdSalary<>'1'";
...
while($r=mysql_fetch_assoc($exec)){
		//$grossPay = $r['netBasic'] + $r['cola'] + $r['allowance'] + $r['honorarium'] + $r['payNightPremium'] + $r['payOTReg'] + $r['oth_income'] + $r['onCallOvertime'] + $r['payDutyRd'] + $r['paySpHoliday'] + $r['payLHoliday'] + $r['payOTExc'] - $r['payUndertime'] +  $r['adj_other'] + $r['otRDLHolidayPay'] + $r['otRDSHolidayPay'] + $r['otLHolidayPay'] + $r['otSHolidayPay'] + $r['otRestDayPay'];
     ...
     <table width="700" style="font-family:Arial;font-size:12px;">
	  ...
	  <tr>
	       <td colspan="4" align="left" style="font-size:14px;"><?php echo date('d F Y');?><br><br> 
						...
						Please debit our Savings Acount No. 3-66701583-0 the total amount of Pesos: P &nbsp;&nbsp;<?php echo number_format($totalNetPay,2);?> and credit to the following accounts
