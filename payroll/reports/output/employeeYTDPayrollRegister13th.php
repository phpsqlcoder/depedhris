...
while($rs=mysql_fetch_assoc($exec)){
	...
	$grossPay = $r['netBasic'] + $r['cola'] + $r['allowance'] + $r['incentive']  + $r['hazardPay'] + $r['honorarium'] + $r['payNightPremium'] + $r['payOTReg'] + $r['payOTExc'] + $r['oth_income'] + $r['onCallOvertime'] + $r['payDutyRd'] + $r['paySpHoliday'] + $r['payLHoliday'] + $r['otRDLHolidayPay'] + $r['otRDSHolidayPay'] + $r['otLHolidayPay'] + $r['otSHolidayPay'] + $r['otRestDayPay'] +  $r['adj_other'];
	...
	$data.="<tr bgcolor='".$bgclr1s."' align='right'>
					      ...
					      <td>".number_format($r['basicpay'],2)."&nbsp; </td>
						   	<td>".number_format(($r['netBasic'] + $colaUndertimeAmount),2)."&nbsp; </td>
	...
	$grandTotalNetBasic += $r['netBasic'] + $colaUndertimeAmount;
