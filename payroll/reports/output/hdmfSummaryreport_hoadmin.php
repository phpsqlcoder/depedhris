...
function cutline($filename,$line_no=-1) {   	// BEGIN funtion for delete file.txt per line
	...
	$sql = "select SUM(p.pagibig + p.pagibigSavings) hdmfTotalPerDept, sum(100) employerShare,  CASE
	when e.level>3 THEN 'HO ADMIN' ELSE d.name END as departmentName,
			SUM(CASE WHEN p.grossPay > 14750 THEN 30 else 10 END) ECConPerDept
				from payroll p 
					...
					where date_format(p.pay_period,'%Y-%m')='".date('Y-m',strtotime($_POST['monthyear']))."' and p.basicpay>0 and e.residencyTrainingProgram<>'ROD'
							Group BY  CASE
	when e.level>3 THEN 'HO ADMIN' ELSE d.name END order by  CASE
	...
	while($r=mysql_fetch_assoc($rs)){
		...
		$data .="<tr align='left'>
					...
					<td align='right'>".number_format($r['hdmfTotalPerDept'] ,2)."</td>
					<td align='right'>".number_format($r['employerShare'],2)."</td>
					<td align='right'>".number_format($r['hdmfTotalPerDept'] + $r['employerShare'],2)."</td> 
				</tr>";
				
		$totalEshare += $r['hdmfTotalPerDept'];
		$totalCshare += $r['employerShare'];
		$totalpC += $r['hdmfTotalPerDept'] + $r['employerShare'];
	}
if($_POST['eksel']=='on'){
		$filename ="hdmfSummaryreport.xls";
...
<body topmargin=0>	
		...
		<thead>
		<tr><td colspan="6">HDMF Remittance Report<br><br></td></tr>
