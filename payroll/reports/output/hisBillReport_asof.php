...
//$paid = get_paid_per_transactionId($r['hnd']);
	$total_paid_q = mysql_fetch_array(mysql_query("select sum(amountPaid) as paid from ar_hospital_ee_payment_ledger where ar_hospital_ee_trx_Id='".$r['hnd']."' and paymentDate<='".$_POST['asof']."'"));
	if(!$total_paid_q){
		$paid=0;
	}else{
		$paid = $total_paid_q['paid'];
	}	
    $var++;
    $ctr1s++;
	$ln++;
	 ...
	 <thead>
	  <tr>
	       <td colspan="24" align="center" style="font-size:14px;font-weight:bold;">DAVAO DOCTORS HOSPITAL<br>HOSPITAL BILL BALANCE REPORT<br /><?php echo $reportTitle;?><br>
				 				As of:<?php echo date('F d, Y',strtotime($_POST['asof']));?></td> </td>
	  </tr>
	  <tr><td>&nbsp;</td></tr>
