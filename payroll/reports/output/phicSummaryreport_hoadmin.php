...
function cutline($filename,$line_no=-1) {   	// BEGIN funtion for delete file.txt per line
	...
	$sql = "select SUM(p.d_philhealth) phicTotalPerDept, CASE
	...
	while($r=mysql_fetch_assoc($rs)){
		...
		$data .="<tr align='left'>
					...
					<td align='right'>".number_format($r['phicTotalPerDept'] ,2)."</td>
					<td align='right'>".number_format($r['phicTotalPerDept'],2)."</td>
					<td align='right'>".number_format($r['phicTotalPerDept']*2,2)."</td>
				</tr>";
				
		$totalEshare += $r['phicTotalPerDept'];
		$totalCshare += $r['phicTotalPerDept'];
		$totalpC += $r['phicTotalPerDept']*2;
	}
if($_POST['eksel']=='on'){
		$filename ="phicSummaryreport.xls";
