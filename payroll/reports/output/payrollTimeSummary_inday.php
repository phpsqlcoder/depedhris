...
$sql = "SELECT e.employmentStatus,e.employeeNo,e.lastName, e.firstName, e.middleName, 
	SUM(p.undertime) as undertime,
	SUM(p.days_work) as days_work,
	SUM(p.days_absent) as days_absent,
	SUM(p.undertime) as undertime,
	SUM(p.duty_rd) as duty_rd,
	SUM(p.adj_duty_rd) as adj_duty_rd,
	SUM(p.ot_reg) as ot_reg,
	SUM(p.ot_exc) as ot_exc,
	SUM(p.otLHoliday) as otLHoliday,
	SUM(p.otSHoliday) as otSHoliday,
	SUM(p.otRestDay) as otRestDay,
	SUM(p.otRDLHoliday) as otRDLHoliday,
	SUM(p.otRDSHoliday) as otRDSHoliday,
	SUM(p.lholiday) as lholiday,
	SUM(p.spholiday) as spholiday,
	SUM(p.bday_lve) as bday_lve,
	SUM(p.night_prem) as night_prem,
	SUM(p.adj_night_prem) as adj_night_prem,
	SUM(p.adj_days_work) as adj_days_work,
	SUM(p.adj_undertime) as adj_undertime,
	SUM(p.adj_sick_lve) as adj_sick_lve,
	SUM(p.adj_vac_lve) as adj_vac_lve,
	SUM(p.adj_official_lve) as adj_official_lve,
	SUM(p.adj_bday_lve) as adj_bday_lve,
							 d.name departmentName
									FROM payroll p left join employee e on e.ndex=p.empid
										
										LEFT JOIN dept d ON d.ndex = e.deptId 
												WHERE p.pay_period>='".$_POST['PayrollCutoffstart']."'  && p.pay_period<='".$_POST['PayrollCutoffend']."' && e.residencyTrainingProgram=''
												";
...
$sql.=" 
GROUP BY e.employmentStatus,e.employeeNo,e.lastName, e.firstName, e.middleName
