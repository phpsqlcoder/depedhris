...
$qry="SELECT * FROM sap ORDER BY name";
if($_GET['act']=='search'){
	if($_POST['ser']==1){
		$qry.=" WHERE name like '%".$_POST['serts']."%'";
	}
	elseif($_POST['ser']==2){
		$qry.=" WHERE sapId = '".$_POST['serts']."'";
	}
	
}
elseif($_GET['act']=='editdept'){
	$qr=mysql_fetch_object(mysql_query("SELECT d.* FROM sap d where d.ndex=".$_GET['id'].""));
}
elseif($_GET['act']=='adnew'){
	$qr=mysql_query("insert into sap (name,sapId)values('".$_POST['description']."','".$_POST['division']."')");
	header("Location:sapdept.php");
	//echo $_POST['division'];
}
elseif($_GET['act']=='saveedit'){
	$qrys=mysql_query("update sap set name='".mysql_real_escape_string($_POST['description'])."',sapId='".$_POST['division']."' WHERE ndex='".$_POST['ndex']."'");
	header("Location:sapdept.php");
}
$cat=mysql_query($qry);
while($c=mysql_fetch_object($cat)){
	...
	$data.="<tr style='font-size:12px;background-color:".$bgclr1s.";color:black;'>
				...
				<td>".$c->name."</td>
				<td>
					<a href='#' onclick=\"window.location.href='sapdept.php?act=editdept&id=".$c->ndex."';\"><img src='images/edit.png' title='Edit dept' height='20px;' width='20px;'></a>
...
<div id="rcont">
  <h2><strong>SAP Department Management</strong></h2>
  <table width="831" border="0">
	
    <tr>
	  <td>&nbsp;</td>
      <td>Search Here:</td>
      <td><form id="form1" name="form1" method="post" action="sapdept.php?act=search">
              <select name="ser">
                <option value="1">Description</option>
                <option value="2">SAP ID</option>
              </select>
...
<form name="frmedit" method="post" action="sapdept.php?act=saveedit">
<table>
	<tr>
		<td>SAP ID:</td>
		<td><input type="hidden" name="ndex" value="<?php echo $qr->ndex;?>"><input type="Text" name="division" value="<?php echo $qr->sapId;?>"></td>
	</tr>
	<tr>
		<td>Description:</td>
...
<?php }else{?>
<form name="frmadds" method="post" action="sapdept.php?act=adnew">
<table>
	<tr><td>&nbsp;</td></tr>
	...
	<tr>
		<td>SAP ID:</td>
		<td><input type="Text" name="division" size=60></td>
	</tr>
	<tr>
		<td>Description:</td>
