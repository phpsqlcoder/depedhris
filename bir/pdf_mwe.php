...
if (!file_exists('result/mwe')) {
    mkdir('result/mwe', 0777, true);
}


$dqry=mysqli_query($conn, "select distinct dept from annualization_mwe");
while($dd=mysqli_fetch_array($dqry)){
			if (!file_exists('individual/'.$dd['dept'])) {
			    mkdir('individual/'.$dd['dept'], 0777, true);
			}
			$q = mysqli_query( $conn, "select *,v.bday from annualization_mwe g
			...
			while($r=mysqli_fetch_array($q)){
				...
				// NOn Taxables
				$pdf->SetXY(164,54);
				$pdf->Cell(45,6,number_format($r['BasicStatMinW'],2),0,0,'R');

				$pdf->SetXY(164,61);
				$pdf->Cell(45,6,number_format($r['HolidayPay_20'],2),0,0,'R');

				$pdf->SetXY(164,68);
				$pdf->Cell(45,6,number_format($r['OverTime_21'],2),0,0,'R');

				$pdf->SetXY(164,76);
				$pdf->Cell(45,6,number_format($r['NightDiff_22'],2),0,0,'R');

				$pdf->SetXY(164,83);
				$pdf->Cell(45,6,number_format($r['HazardPay_23'],2),0,0,'R');

				$pdf->SetXY(164,90);
				$pdf->Cell(45,6,number_format($r['13thMoOthers_24'],2),0,0,'R');

				$pdf->SetXY(164,97);
				$pdf->Cell(45,6,number_format($r['DeMinimis_25'],2),0,0,'R');

				$pdf->SetXY(164,104);
				$pdf->Cell(45,6,number_format($r['Statutory_26'],2),0,0,'R');

				$pdf->SetXY(164,111);
				$pdf->Cell(45,6,number_format($r['OtherSal_27'],2),0,0,'R');

				$pdf->SetXY(164,118);
				$pdf->Cell(45,6,number_format($r['Total_NT_28'],2),0,0,'R');
				...
				*/
				...
				//$pdf->SetFont('Arial','B',10);
				$pdf->SetXY(62,102);
				$pdf->Cell(45,6,number_format($r['BasicPerDay_16'],2),0,0,'R');

				$pdf->SetXY(62,109);
				$pdf->Cell(45,6,number_format($r['BasicPerMo_17'],2),0,0,'R');
				...
				*/
				...
				$t21=0;
				$pdf->SetXY(72,190.5);
				$pdf->Cell(35,6,number_format($r['Total_NT_28'],2),0,0,'R');
				...
				$t30=0;
				$pdf->SetXY(72,234);
				$pdf->Cell(35,6,number_format($t30,2),0,0,'R');
				...
				$pdf->SetXY(133,312);
				$pdf->Cell(35,6,"(SGD) ".$r['LastName'].", ".$r['FirstName']." ".$r['MidName']);
				$pdf->Output("F","individual/".$dd['dept']."/".$r['LastName']."_".str_replace("-", "", $r['TIN_No'])."_12312021.pdf");
				//$pdf->Output("F","result/mwe/".str_replace("/","-",$dd['dept']).".pdf");
			}
