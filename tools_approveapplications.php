...
$count_schedule=0;
$count_restday=0;
if($_SESSION['isApprovingOfficer']<>1){die('invalid page');}
...
*/
	...
	foreach($types as $type){
		...
		if (in_array("73", $dar)){
			...
			if($_SESSION['deptId']=='144'){
				$qry.=" WHERE e.isActive=1 ".$adqry." and (d.ndex in (".$_SESSION['deptId'].") OR e.authorizer=".$_SESSION['ndex'].")";	
			}
			else{
				$qry.=" WHERE e.isActive=1 ".$adqry." and ((d.ndex in (".$_SESSION['deptId'].") and e.authorizer=0) OR e.authorizer=".$_SESSION['ndex'].")";	
			}
			
		}
		$qry.=" and k.tayp='".$type."' and k.approve1='".$_GET['aStatus']."' and k.isDisapproved=0";
	...
	//$qry.=" limit 30";
		...
		while($r=mysql_fetch_object($exec)){
			...
			$empid = getID($r->employmentStatus,$r->employeeNo);
			if($empid<>$_SESSION['empid']){
				...
				if($type=='Overtime'){
	    			...
	    			$outlog=mysql_fetch_object(mysql_query("select * from hrinterface where dtrid='".$r->biometricNo."' and in_out=1 and log>'".$inlog->log."' ORDER BY log LIMIT 0,1"));
				...
				if($log['ndex']){
				    ...
				    $date_approved='';
				    $edit_link="
				    <a href='#/'	class='btn btn-xs green' title='Approve' onclick='approve(".$r->ndex.");'><i class='fa fa-check'></i></a>
				    <a href='#/'	class='btn btn-xs red' title='Disapprove' onclick=\"jQuery('#di".$r->ndex."').toggle();\"> <i class='fa fa-times'></i></a>
				    <a href='#/'	class='btn btn-xs purple' title='Edit' onclick=\"window.open('online_apps/".$link."?id=".$r->ndex."','displayWindow','toolbar=no,scrollbars=yes,width=1200,height=500')\";> <i class='fa fa-pencil'></i></a>";
				  }
				...
				<td id='dev".$r->ndex."' width='150'>
					".$edit_link."
					<div style='display:none;' id='di".$r->ndex."'><br>
						<textarea cols='30' rows='5' id='reason".$r->ndex."' name='reason".$r->ndex."' PLACEHOLDER='Reason for Disapproval' required='required'></textarea>
						<a href='#/' onclick='disapprove(".$r->ndex.");' class='btn btn-xs red'>Submit</a>
					</div>
				</td>
				<td>".$date_created."</td>
				...
				<td>".$r->date."</td>
				<td>".$r->request."</td>
				<td width='300'>".$r->remarks."</td>
				<td style='color:red;font-weight:bold;'>".$hist['remarks']."</td>			
				";	
				$data[$type].="</tr>";
			}
		}
...
<body>
	...
	<div id="main_content_wrap" class="container_12" style="width:90%;">
		...
		<div id="rcont">
			<h2><strong>For Approval</strong> <a style="margin-left:50px;" class="btn green" href="tools_approveapplications_history.php">Transactions</a> </h2> 
			...
			?>
				...
				<form action="tools_approveapplications.php" method="get" name="sadsa">
					...
					<small class="pull-left">Legend: <span class="badge badge-warning" style="color:black;font-weight:bold;">1</span>&nbsp;= Unapprove Applications<br><br></small>
					        ...
					        <div class="col-md-12 tab-style-1">
						          <ul class="nav nav-tabs">
						            <li class="active"><a data-toggle="tab" href="#tab-1">
						            	<span class="badge badge-warning" style="color:black;font-weight:bold;"><?php echo $count_overtime; ?></span>&nbsp;Overtime</a></li>
						            <li class=""><a data-toggle="tab" href="#tab-2">
						            	<span class="badge badge-warning" style="color:black;font-weight:bold;"><?php echo $count_restday; ?></span>&nbsp;Duty Restday</a></li>
						            <li class=""><a data-toggle="tab" href="#tab-3">
						            <span class="badge badge-warning" style="color:black;font-weight:bold;"><?php echo $count_log; ?></span>&nbsp;Forgot to Log</a></li>
						            <li class=""><a data-toggle="tab" href="#tab-4">
						            	<span class="badge badge-warning" style="color:black;font-weight:bold;"><?php echo $count_schedule; ?></span>&nbsp;Change Schedule</a></li>
						        	<li class=""><a data-toggle="tab" href="#tab-5">
						        	<span class="badge badge-warning" style="color:black;font-weight:bold;"><?php echo $count_leave; ?></span>&nbsp;Leave</a></li>
						          </ul>
...
<script src="kiosk1/assets/global/plugins/jquery-1.11.0.min.js" type="text/javascript"></script>
    ...
    <script type="text/javascript">
    	...
    	function disapprove(x){

    		
    		if(jQuery('#reason'+x).val()==''){
    			alert('Please enter the reason for Disapproval');
    			return false;
    		}
    		else{
    			jQuery.ajax({
		    		method: "POST",
		    		url: "tools_approveonline_ajax.php?act=disapprove&id="+x+"&remark="+jQuery('#reason'+x).val()
		    	})
		    	.done(function(data){
		    		jQuery('#dev'+x).html("Disapproved");
    	...
    	function approve(x){
    		...
    		var result = confirm("Are you sure you want to APPROVE this request?"); if(result){
    			jQuery.ajax({
		    		method: "POST",
		    		url: "tools_approveonline_ajax.php?act=approve&id="+x
		    	})
		    	.done(function(data){
		    		jQuery('#dev'+x).html("Approved");		    		
		    	});
    		}
