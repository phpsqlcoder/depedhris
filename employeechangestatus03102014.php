...
if($_GET['act']=='adnew'){
	...
	if($_POST['employeestatus']=='Lateral Transfer' || $_POST['employeestatus']=='Promotion' || $_POST['employeestatus']=='Demotion'){
		...
		if($_POST['dept']!='wala'){
			$insdept=mysql_query("insert into employeechangestatus(employeeId,`changeType`, `newValue`, `effectivityDate`,createdBy,createdDate,isDept) VALUES ('".$_GET['id']."','".$_POST['employeestatus']."','".$_POST['dept']."','".$_POST['effectivityDate']."','".$_SESSION['fullName']."','".date('Y-m-d')."',1)");
		}
	}
...
$checkIfExistInCompen = mysql_num_rows(mysql_query($sql = "SELECT * FROM employee_compensation WHERE employeeId='".$_GET['id']."'",$conn));
	...
	$sql = "SELECT e.*, ec.basicPay, ec.allowance, ec.payTypeNdex, ec.honorarium, ec.cola, ec.withoutPremium, ec.taxType,d.name as deptname  FROM employee e
	...
	if ($_GET['pageAction'] == 'updateCompen'){
			$updateTable = mysql_query("UPDATE employee_compensation SET basicPay='".$_POST['basicPay']."', allowance='".$_POST['allowance']."', payTypeNdex='".$_POST['payTypeNdex']."', honorarium='".$_POST['honorarium']."', cola='".$_POST['cola']."', withoutPremium='".$_POST['withoutPremium']."',taxType='".$_POST['taxType']."' WHERE employeeId='".$_POST['id']."'",$conn);
...
$sqry=mysql_query("select * from employeechangestatus where employeeId=".$_GET['id']." and changeType in ('Resignation','End of Contract','Separation','Lateral Transfer',
'employmentStatus','Demotion','Promotion','End of Residency Training','Retirement','Termination','Employment Status') order by effectivityDate");
while($r=mysql_fetch_object($sqry)){
	if($r->changeType=='Lateral Transfer' || $r->changeType=='Promotion' || $r->changeType=='Demotion'){
		$val=mysql_fetch_object(mysql_query("select * from position where ndex='".$r->newValue."'"));
		$newvalue=$val->name;
	}
	else{
		$newvalue=$r->newValue;
...
<?php if($_SESSION['ndex']=='15'){$hyd='block';}else{$hyd='none';}?>
...
<table style="font-family:Arial;font-size:12px;">
	...
	<tr><td><strong>Status:</strong></td><td>
		...
		<select name="dept" id="dept" style="display:none;"><option value="wala">- Select Dept -<?php echo $optiondept;?></select>
