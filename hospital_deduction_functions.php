...
function get_amortization($Batch_No, $AR_No, $empid){
	global $conn;	
	$amortization = mysql_fetch_array(mysql_query("select * from hospital_deduction_amortization where employeeId='".$empid."' and AR_No = '".$AR_No."' and Batch_No='".$Batch_No."' and Status='Active'"));
...
function get_payables($Batch_No,$AR_No, $empid){
	global $conn;
	$total_bill = mysql_fetch_array(mysql_query("select sum(Amount) as bills from ar_hospital_ee_trx where employeeId='".$empid."'  and AR_No = '".$AR_No."' and Batch_No='".$Batch_No."' and Status='Active'"));

	if(!$total_bill)
		return 0;
	else
		return $total_bill['bills'];
}
