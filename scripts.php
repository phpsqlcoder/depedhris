...
///start jo
function searchproductjo(act,dev,myform){
	var nxtid=$(snxtid).value;
		/*$('sforhid').value=$('sfor').value;
		$('sfor').disabled=true;*/
		mynodes=myform.serialize();	
...
function inserttotempjo(act,dev,myform){
	mynodes=myform.serialize();	
	new Ajax.Updater(dev,'actions.php?acts='+act,{
		method: 'get',
		onLoading: function(){$(dev).innerHTML='<img src="images/ajax.gif">'},
		onSuccess: function(){
					var nxtid=$(snxtid).value;
					mynodes=myform.serialize();	
					new Ajax.Updater('selectedproducts','actions.php?acts=getFromTempjo&nxtid='+nxtid,{
					method: 'get',
					parameters: mynodes
				});
				},
...
function dbconjo(act,dev,myform){	
	mynodes=myform.serialize();	
	new Ajax.Updater(dev,'actions.php?acts='+act,{
		method: 'get',
		onLoading: function(){$(dev).innerHTML='<img src="images/ajax.gif">'},
...
function warn(inv,inp,ting){
	if(parseFloat(inp)>parseFloat(inv)){
