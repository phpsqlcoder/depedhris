...
if ($_GET['pageact'] == "runCutoff"){
	...
	$sqlUpd = "SELECT * FROM dailytimesummary WHERE date BETWEEN '".$getInfoCutoffDate['cutoffDateStart']."' AND '".$getInfoCutoffDate['cutoffDateEnd']."' and employeeId='1344'";
...
//		die();
	...
	//if($checkIfExistInPayroll == 0){
		$sql= "SELECT ndex FROM employee WHERE isActive='1' and ndex='1344'";
	...
	/*if (isErrorTimeSummary($getInfoCutoffDate['cutoffDateStart'],$getInfoCutoffDate['cutoffDateEnd']) == 1){
		...
		$sql = "SELECT e.ndex as employeeId, SUM(dts.days_absent) daysAbsent, SUM(dts.minutesLate + dts.undertime) undertime, SUM(dts.ot_reg) otReg, SUM(dts.ot_exc) otExc, SUM(dts.days_work) daysWork
											...
											WHERE e.isActive='1' && dts.date BETWEEN '".$getInfoCutoffDate['cutoffDateStart']."' AND '".$getInfoCutoffDate['cutoffDateEnd']."' and e.ndex='1344'
...
<div id="main_content_wrap" class="container_12">
    ...
    <div class="clearfix">
			<form method="POST" action="b.php?pageact=runCutoff" margin="0px;" name="myForm">
