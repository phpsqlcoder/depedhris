...
############# End of End Time #################
	$startDateOnly=date('m-d',strtotime($start));
	if($startDateOnly=='03-07'){date_default_timezone_set("Asia/Manila");}
...
if(date('H',$start_work) == '00'){
		$start_night = mktime('18','00','00',date('m',$start_work),date('d',$start_work) - 1,date('Y',$start_work));
		$end_night   = mktime('06','00','00',date('m',$start_work),date('d',$start_work),date('Y',$start_work));
	}
					...
					if($ee->ndex){
						...
						if($chkifexist>=1){						
							if($check_duplicate_record==100){
								...
								$insert=mysql_query("update `dailytimesummary` set `isError`=".$check_duplicate_record.",`undertime`=".$undertime.",`minutesLate`=".$late.",`night_prem`=".$nytdifferential.",`holiday`='".$holidays."',`days_work`='".$daysWork."'".$absentvar." where `employeeId`='".$ee->ndex."' and `date`='".$rs->datelog."'");
							}
							else{
								/*
								...
								$insert=mysql_query("update `dailytimesummary` set `hoursDuty`=".$hours_duty.",`isError`=".$check_duplicate_record.",`undertime`=".$undertime.",`minutesLate`=".$late.",`night_prem`=".$nytdifferential.",`holiday`='".$holidays."',`days_work`='".$daysWork."'".$absentvar." where `employeeId`='".$ee->ndex."' and `date`='".$rs->datelog."'");
...
function philHelthPremium ($basicPay){
	...
	$minPrem = 500;
	$maxPrem = 5000;
	$percentMultiplier  = 0.05;	

	$totalShare = (($basicPay * $percentMultiplier) < $minPrem ? $minPrem: $basicPay * $percentMultiplier) > $maxPrem ? $maxPrem : (($basicPay * $percentMultiplier) < $minPrem ? $minPrem: $basicPay * $percentMultiplier);
	return array(	
		eeShare => $totalShare / 2,
		companyShare => $totalShare / 2
	);
}
 
/* updated on jan 25 2024 by jundrie
function philHelthPremium ($basicPay){
	//$sql = "SELECT * FROM tbl_philhealth  WHERE income >'".$basicPay."' ORDER BY ndex ASC limit 1";
	//$rs = mysql_fetch_assoc(mysql_query($sql));
	$minPrem = 400;
	$maxPrem = 3200;
	$percentMultiplier  = 0.04;	
...
*/
/* updated on june 24, 2022 by jundrie
function philHelthPremium ($basicPay){
	//$sql = "SELECT * FROM tbl_philhealth  WHERE income >'".$basicPay."' ORDER BY ndex ASC limit 1";
	//$rs = mysql_fetch_assoc(mysql_query($sql));
	$minPrem = 300;
	$maxPrem = 1800;
	$percentMultiplier  = 0.03;	
...
function withHeldTax ($dependent, $income, $dedFrequency='MONTHLY'){
	$sql = "SELECT * FROM tbl_wtax10 LEFT JOIN tbl_wtax11 ON tbl_wtax10.ndex=tbl_wtax11.ndexwt10 
...
function netTaxableCompensation ($netTaxable){
	$sql = "SELECT * FROM tbl_annualtax WHERE '".$netTaxable."' BETWEEN over AND butNotOver";
	$rs = mysql_query($sql);
	$dt = mysql_fetch_assoc($rs);
	$netTaxableCompensation = $dt['taxAmount'] + ( ($netTaxable - $dt['excessOverAmount']) * ($dt['excessPercentage'] / 100) );
