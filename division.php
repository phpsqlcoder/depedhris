...
elseif($_GET['act']=='editdept'){
	$qr=mysql_fetch_object(mysql_query("SELECT * FROM division where ndex=".$_GET['id'].""));
}
elseif($_GET['act']=='adnew'){
	$qr=mysql_query("insert into division (name,head)values('".$_POST['description']."','".$_POST['head']."')");
	header("Location:division.php");
}
elseif($_GET['act']=='saveedit'){
	$qrys=mysql_query("update division set name='".mysql_real_escape_string($_POST['description'])."',head='".mysql_real_escape_string($_POST['head'])."' WHERE ndex='".$_POST['ndex']."'");
	header("Location:division.php");
}
$cat=mysql_query($qry);
while($c=mysql_fetch_object($cat)){
	...
	$data.="<tr style='font-size:12px;background-color:".$bgclr1s.";color:black;'>
				...
				<td>".$c->head."</td>
				<td>
					<a href='#' onclick=\"window.location.href='division.php?act=editdept&id=".$c->ndex."';\"><img src='images/edit.png' title='Edit supplier' height='20px;' width='20px;'></a>
...
<div id="rcont">
  <h2><strong>Division Management</strong></h2>
  <table width="831" border="0">
	
    <tr>
	  <td>&nbsp;</td>
      <td>Search Here:</td>
      <td><form id="form1" name="form1" method="post" action="division.php?act=search">
              <select name="ser">
...
<form name="frmedit" method="post" action="division.php?act=saveedit">
<table>
	<tr>
		<td>Description:</td>
	...
	<tr>
		<td>Head:</td>
		<td><input type="Text" name="head" size=60 value="<?php echo $qr->head;?>">
...
<?php }else{?>
<form name="frmadds" method="post" action="division.php?act=adnew">
<table>
	<tr><td>&nbsp;</td></tr>
	
	<tr><td><h1>Add new division</h1></td></tr>
	<tr>
		<td>Description:</td>
