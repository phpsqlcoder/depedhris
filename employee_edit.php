...
if($_GET['act']=='save'){
	...
	while($q=mysql_fetch_object($t)){
		...
		if($_POST[$q->Field]){
			//echo $q->Field." xx ".$_POST[$q->Field]."<br>";
			if(!in_array($_POST[$q->Field],$nottoinclude)){
				...
				if(!$_POST['bankAccountNo'] || $_POST['bankAccountNo']==""){
					$_POST['bankAccountNo']=" ";
				}
				// if(!$_POST['unitId'] || $_POST['unitId']==""){
				// 	$_POST['unitId']=0;
				// }
				
				$_POST['allergicFood']='';
				$_POST['allowedFood']='';
				if(!empty($_POST['allowedFoods'])) {
				    foreach($_POST['allowedFoods'] as $check) {
				        $_POST['allowedFood'].=$check."|";
				    }
				}
				$_POST['allowedFood']=rtrim($_POST['allowedFood'],"|");

				if(!empty($_POST['allergicFoods'])) {
				    foreach($_POST['allergicFoods'] as $check) {
				        $_POST['allergicFood'].=$check."|";
				    }
				}				
				if(!empty($_POST['otherAllergicFoods'])) {
					$_POST['allergicFood'].=$_POST['otherAllergicFoods']."|";
				}
				$_POST['allergicFood']=rtrim($_POST['allergicFood'],"|");

				//if($_POST['level']==100){$_POST['level']='0';}	
				$oldrecord=mysql_fetch_array(mysql_query("select * from employee where ndex=".$id.""));
				//echo $q->Field." xxx ".$oldrecord[$q->Field]."<br>";
...
//echo $param;
	...
	$updateemployeeunit=mysql_query("UPDATE employee SET unitId='".$_POST['unitId']."' where ndex=".$id."");
	...
	for($dd=1;$dd<=$_POST['trainingcounter'];$dd++){
		$inserttraining=mysql_query("
			...
			VALUES ('".$id."','".$_POST['nametraining'.$dd]."','".$_POST['fromDateprof'.$dd]."','".$_POST['endDateprof'.$dd]."','".$_POST['venue'.$dd]."')
		");
	}
	//interest
	...
	//files
	$path = "empfiles/".$id."/"; // Upload directory
	if (!file_exists($path)) {
	    mkdir($path, 0777, true);
	}	
	$count = 0;
	// Loop $_FILES to exeicute all files
	foreach ($_FILES['empfiles']['name'] as $f => $name) {    
	            if(move_uploaded_file($_FILES["empfiles"]["tmp_name"][$f], $path.$name))
	 	...
	 	<tr>
			<td>
				<table>
				 	<tr>
						<td>Full Name:&nbsp;&nbsp;</td>
						<td><input type="Text" name="firstName" size="15" value="<?php echo utf8_decode($em->firstName);?>">
						<input type="Text" name="middleName" size="15" value="<?php echo utf8_decode($em->middleName);?>">
						<input type="Text" name="lastName" size="15" value="<?php echo utf8_decode($em->lastName);?>"></td>
					</tr>
					...
					<tr><td>Department:&nbsp;&nbsp;</td><td><div id="deptdiv"><select name="deptId" onchange="showEffectivityDate('deptIdDatediv');"><option value="">- Select Division -<?php echo $optdept;?></select></div>
							<div id="deptIdDatediv" style="font-size:10p;display:none;"><strong style="color:blue;">Effectivity Date:</strong><input type="Text" name="deptIdDate" id="deptIdDate" size="15" value="<?php echo date('Y-m-d');?>"><a href="javascript:show_calendar('employeefrm.deptIdDate');" onMouseOver="window.status='Date Picker'; overlib(''); return true;" onMouseOut="window.status=''; nd(); return true;"><img src="b_calendar.png" width=19 border=0></a></div>
							</td></tr>
					<tr><td>Unit:&nbsp;&nbsp;</td><td id="unitdiv"><div id="unitdiv3"><select name="unitId" id="unitId">
<option value="0">- Select Department -<?php echo $optunit;?></select></div></td></tr>
					...
					<tr><td>Date Hired:&nbsp;&nbsp;</td><td><input type="Text" name="dateHired" id="dateHired" value="<?php echo $em->dateHired;?>" size="15"><a href="javascript:show_calendar('employeefrm.dateHired');" onMouseOver="window.status='Date Picker'; overlib(''); return true;" onMouseOut="window.status=''; nd(); return true;"><img src="b_calendar.png" width=19 border=0></a></td></tr>
					<tr><td>Bank Account No:&nbsp;&nbsp;</td><td><input type="Text" name="bankAccountNo" <?php if($_SESSION['ndex']!='15' && $_SESSION['ndex']!='22' && $_SESSION['ndex']!='14' && $_SESSION['ndex']!='273' && $_SESSION['ndex']!='12' ){echo "readonly='readonly'";}?> value="<?php echo $em->bankAccountNo;?>" size="15"></td></tr>
					...
					<tr><td>Authorizer:&nbsp;&nbsp;</td><td><select name="authorizer"><?php echo $optionusers2;?><option value="<?php echo $em->authorizer;?>" selected="selected"><?php if($em->authorizer==0) {echo '- None -';} else { 
						$uuu = mysql_fetch_array(mysql_query("select * from users where ndex='".$em->authorizer."'"));
						echo $uuu['fullName']." (".$uuu['nym'].")"; }?></select></td></tr>
					<tr><td>Medical Allowance:&nbsp;&nbsp;</td><td><input type="number" name="medical_allowance"value="<?php echo $em->medical_allowance;?>" step="0.01"></td></tr>
	 ...
	 <div class="row">
        ...
        <div class="col-md-12 tab-style-1">
          <ul class="nav nav-tabs">
            ...
            <li class=""><a data-toggle="tab" href="#tab-2">Personal Background</a></li>
            <li class=""><a data-toggle="tab" href="#tab-3">Educational Background</a></li>
            <li class=""><a data-toggle="tab" href="#tab-4">Employment Background</a></li>
        	<li class=""><a data-toggle="tab" href="#tab-5">Docs</a></li>
          </ul>
...
Expiration:<input value="<?php echo $em->prcDateExpiration;?>" type="Text" name="prcDateExpiration" id="prcDateExpiration" size="15"><a href="javascript:show_calendar('employeefrm.prcDateExpiration');" onMouseOver="window.status='Date Picker'; overlib(''); return true;" onMouseOut="window.status=''; nd(); return true;"><img src="b_calendar.png" width=19 border=0></a>
            ...
            <div id="tab-4" class="tab-pane fade">
             ...
             <div id="tab-5" class="tab-pane row fade">
              <div class="col-md-12">
                <table width="100%">
                	<tr><td><strong style="font-size:12px;color:maroon;"><u>SCANNED DOCUMENTS</u></strong></td></tr>
					<tr><td>&nbsp;</td></tr>	
					<tr>
						<td>
							<input name="empfiles[]" id="empfiles" type="file" multiple="" />				
						</td>
					</tr>
					<tr>
						<td>
							<table>
								<iframe src="emp_upload_files.php?id=<?php echo $_GET['id'];?>" height="300" width="100%"></iframe>	
							</table>			
						</td>
					</tr>
              	</table>
 ...
 <br><br> <br><br>
         <div class="col-md-12 tab-style-1">
          <ul class="nav nav-tabs">
            <li class="active"><a data-toggle="tab" href="#tab-6">Meritorious/Honors Bg</a></li>         
            <li class=""><a data-toggle="tab" href="#tab-7">Continuous Professional Development</a></li>
            <li class=""><a data-toggle="tab" href="#tab-8">Technical/Technological Proficiency</a></li>
            <li class=""><a data-toggle="tab" href="#tab-9">Professional Information</a></li>
		<li class=""><a data-toggle="tab" href="#tab-10">Meals</a></li>
          </ul>
          <div class="tab-content">
            <div id="tab-6" class="tab-pane row fade active in">
              <div class="col-md-12">
                <table width="100%">
            ...
            <div id="tab-7" class="tab-pane row fade">
              <div class="col-md-12">
                <table width="100%">
            ...
            <div id="tab-8" class="tab-pane row fade">
              <div class="col-md-12">
                <table width="100%">
...
<div id="tab-10" class="tab-pane row fade">
              <div class="col-md-12">
                <table width="100%">
                	<tr><td><strong style="font-size:12px;color:maroon;"><u>Vlll. MEALS </u></strong></td></tr>
					<tr><td>&nbsp;</td></tr>	
					<tr>
						<td>
							<?php 
								$allowedFoods = explode("|", $em->allowedFood);
								$allergicFoods = explode("|", $em->allergicFood);
								$def_array = array("Pork","Fish","Beef","Chicken");
								$otherallergicfood='';
								foreach($allergicFoods as $af){
									if (!in_array($af, $def_array)){
										$otherallergicfood.=$af;
									}
								}
							?>
							<table width="80%">
								<tr>
									<td><input type="Checkbox" name="allowedFoods[]" <?php if (in_array("Pork", $allowedFoods)){echo "checked='checked'";}?> value="Pork"/>Pork</td>
									<td><input type="Checkbox" name="allowedFoods[]" <?php if (in_array("Fish", $allowedFoods)){echo "checked='checked'";}?> value="Fish"/>Fish</td>
									<td><input type="Checkbox" name="allowedFoods[]" <?php if (in_array("Beef", $allowedFoods)){echo "checked='checked'";}?> value="Beef"/>Beef</td>
									<td><input type="Checkbox" name="allowedFoods[]" <?php if (in_array("Chicken", $allowedFoods)){echo "checked='checked'";}?> value="Chicken"/>Chicken</td>
								</tr>

								<tr><td style="font-weight:bold;"><br><br>Allergies</td></tr>
								<tr>
									<td><input type="Checkbox" name="allergicFoods[]" <?php if (in_array("Pork", $allergicFoods)){echo "checked='checked'";}?> value="Pork"/>Pork</td>
									<td><input type="Checkbox" name="allergicFoods[]" <?php if (in_array("Fish", $allergicFoods)){echo "checked='checked'";}?> value="Fish"/>Fish</td>
									<td><input type="Checkbox" name="allergicFoods[]" <?php if (in_array("Beef", $allergicFoods)){echo "checked='checked'";}?> value="Beef"/>Beef</td>
									<td><input type="Checkbox" name="allergicFoods[]" <?php if (in_array("Chicken", $allergicFoods)){echo "checked='checked'";}?> value="Chicken"/>Chicken</td>
									<td>Others (Pls specify): <input type="text" name="otherAllergicFoods" value="<?php echo $otherallergicfood; ?>" />	 </td>
								</tr>
								<!-- <tr><td>Pork Eater:&nbsp;&nbsp;</td><td><input type="Radio" name="nonPorkEater" value="0" <?php if($em->nonPorkEater==0){echo "checked='checked'";}?>>Yes<input type="Radio" name="nonPorkEater" value="1" <?php if($em->nonPorkEater==1){echo "checked='checked'";}?>>No</td></tr>	
								<tr><td>Beef Eater:&nbsp;&nbsp;</td><td><input type="Radio" name="nonBeefEater" value="0" <?php if($em->nonBeefEater==0){echo "checked='checked'";}?>>Yes<input type="Radio" name="nonBeefEater" value="1" <?php if($em->nonBeefEater==1){echo "checked='checked'";}?>>No</td></tr>	
								<tr><td>Allergic to Chicken:&nbsp;&nbsp;</td><td><input type="Radio" name="chickenAllergic" value="1" <?php if($em->chickenAllergic==1){echo "checked='checked'";}?>>Yes<input type="Radio" name="chickenAllergic" value="0" <?php if($em->chickenAllergic==0){echo "checked='checked'";}?>>No</td></tr>	
								<tr><td>Allergic to Seafoods:&nbsp;&nbsp;</td><td><input type="Radio" name="seafoodsAllergic" value="1" <?php if($em->seafoodsAllergic==1){echo "checked='checked'";}?>>Yes<input type="Radio" name="seafoodsAllergic" value="0" <?php if($em->seafoodsAllergic==0){echo "checked='checked'";}?>>No</td></tr>	
              				 --></table>
              </div>
            </div>
        ...
        <input type="Submit" value="Update Record" class="btn green">
    	</div>
    	</form> 
        <!-- END TABS -->
	<h2>&nbsp;</h2>
