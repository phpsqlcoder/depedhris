...
for ( $i = $start; $i <= $end; $i += 86400 ){
	...
	if($log->isDayOff>0){ 
		$totalnytpremium+=$log->night_prem;
		$data.="<tr style='background-color:".$bgclr1s.";' onclick=\"Effect.toggle('".$datelog."', 'blind', { duration: 1.0 });\">
					<td>".$datelog."(".date('D',strtotime($datelog)).")</td>
					<td>".$shiftsked->shft."&nbsp;&nbsp;&nbsp;".$hol."</td>
					<td>".$timeLogs." ".$bd."</td>
...
//		echo "SELECT * from employee_leave where employeeId='".$_GET['id']."' and leaveId='".$lev->ndex."' where startDate>='".date('Y'.'-01-01')."' and endDate<='".date('Y'.'-12-31')."' order by startDate";
