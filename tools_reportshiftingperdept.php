...
$deptqry=mysql_query("select * from dept where ndex in (".$_SESSION['deptId'].")");
	//echo "select * from dept where ndex in (".$_SESSION['deptId'].")";
	while($dp=mysql_fetch_object($deptqry)){
		$depsel.="<option value='".$dp->ndex."'>".$dp->name."";
	}
...
if($_GET['startDate']){
	...
	for ( $ab = $start; $ab <= $end; $ab += 86400 ){
		$detb=date('Y-m-d',$ab);
		$data.="<td align='center'>".date('d',strtotime($detb))."<br>(".date('D',strtotime($detb)).")</td>";
	}
	...
	$qry.=" WHERE e.isActive=1 and d.ndex =".$_GET['deps']."";
...
<div id="overDiv" style="position:absolute;right: 220px; top: 200px; visibility:hidden; z-index:1000;"></div>
	...
	?>
	 ...
	 if(!$_GET['startDate']){
	 ?>
	  <form action="tools_reportshiftingperdept.php" method="get" name="sadsa">
	 ...
	 <tr>
		...
		<td>Dept:<select name='deps'><?php echo $depsel;?></select></td>
...
if($_GET['eksels']=='on'){
	...
	<table width="100%" style="font-family:Arial;font-size:12px;">
	  ...
	  <tr><td colspan='2'>&nbsp;</td><td colspan="25" align="right">Date: <?php echo date('F d',strtotime($_GET['startDate']))." - ".date('F d, Y',strtotime($_GET['endDate'])); ?></td></tr>
	  <tr><td>&nbsp;</td></tr>
	  <?php echo $data;?>
	 </table>
