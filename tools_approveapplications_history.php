...
*/
	...
	if(!$_GET['mant']){
		...
		$_GET['depsel']='all';
		$_GET['stat']='all';
		$stat="<option value='all' selected='selected'> - All -</option>";
		...
		$_GET['startDate']=$cutoffq['cutoffDateStart'];
		$_GET['endDate']=$cutoffq['cutoffDateEnd'];
		$stat="<option value='".$_GET['stat']."' selected='selected'>".$_GET['stat']."</option>";
	}
	...
	foreach($types as $type){
		...
		if($_GET['stat']!='all'){
			...
			if($_GET['stat']=='Approved'){
				$qry.= " and k.approve1='1'";
			}
			if($_GET['stat']=='Disapproved'){
				$qry.= " and k.isDisapproved='1'";
			}
		}
		else{
			$qry.=" and (k.approve1='1' OR k.isDisapproved='1')";
		}
	...
	//$qry.=" limit 30";
		...
		while($r=mysql_fetch_object($exec)){
			...
			$is_cb='';
			  if($r->isDisapproved == '1'){
			    $is_cb='Disapproved';
			  }
			  else{
			    $is_cb = ($r->approve1 == '1' ? 'Approved' : '');
			  }
			$addr = '';
			$link="";
			if($type=='Overtime'){
    			...
    			";
    			if($r->approve1 == '1') { $count_overtime++; }
			}
			if($type=='drd'){
				$link="drd_edit.php";
				$inlog=mysql_fetch_object(mysql_query("select * from hrinterface where dtrid='".$r->biometricNo."' and datelog='".$r->date."' and in_out=0 ORDER BY log LIMIT 0,1"));
			...
			if($type=='Schedule'){
    			...
    			New = ".$n."		
    			";
    			if($r->approve1 == '1') { $count_schedule++; }
			}
			if($type=='leave'){
				$link="leave_edit.php";
				$exp = explode("|",$r->request);
...
<body>
	...
	<div id="main_content_wrap" class="container_12" style="width:90%;">
		...
		<div id="rcont">
			<h2><strong>Transactions</strong> <a style="margin-left:50px;" class="btn green" href="tools_approveapplications.php">For Approval</a><h2>
			...
			?>
				...
				<form action="tools_approveapplications_history.php" method="get" name="sadsa">
						...
						<tr>
							<td colspan="5">
								<table><tr>
									...
									<td>Status:<select name="stat" id="stat">
										<option value="all">- All -</option>
										<option value="Approved">Approved</option>
										<option value="Disapproved">Disapproved</option>
										<?php echo $stat;?></select></td>
