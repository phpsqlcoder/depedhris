...
if($_GET['act']=='aprob'){
	if (in_array("73", $dar)){
		//echo "1";
		$aprovsh=mysql_query("update employee_shifting e left join employee em on em.ndex=e.employeeId set approvedBy='".$_SESSION['fullName']."',approvedDate='".date('Y-m-d H:i:s')."' where e.startDate>='".$_GET['startDate']."' and e.endDate<='".$_GET['endDate']."' and (em.approvingOfficer=".$_SESSION['ndex']." OR em.deptId in (".removear($dar)."))");
		$aprovsh=mysql_query("update employee_restday e left join employee em on em.ndex=e.employeeId set approvedBy='".$_SESSION['fullName']."',approvedDate='".date('Y-m-d H:i:s')."' where e.startDate>='".$_GET['startDate']."' and e.endDate<='".$_GET['endDate']."' and (em.approvingOfficer=".$_SESSION['ndex']." OR em.deptId in (".removear($dar)."))");
		$aprovsh=mysql_query("update employee_leave e left join employee em on em.ndex=e.employeeId set approvedBy='".$_SESSION['fullName']."',approvedDate='".date('Y-m-d H:i:s')."' where e.startDate>='".$_GET['startDate']."' and e.endDate<='".$_GET['endDate']."' and (em.approvingOfficer=".$_SESSION['ndex']." OR em.deptId in (".removear($dar)."))");
	}
	else{
		//echo "2";
...
if($_GET['startDate']){
	...
	$qry="SELECT e.firstName,e.lastName,e.middleName,p.name as position,d.name as dept,e.ndex as empid from employee e left join position p on p.ndex=e.position left join dept d on d.ndex=e.deptId";
	if (in_array("73", $dar)){
		$qry.=" WHERE e.isActive=1 ".$adqry." and (e.approvingOfficer=".$_SESSION['ndex']." OR e.deptId in (".removear($dar)."))";
	}
	else{
