...
$errl='';
function getleavebal($empid,$leave){
	$levqry=mysql_query("select * from `leave` where ndex not in(5,13) and ndex='".$leave."' order by name");
	while($lev=mysql_fetch_object($levqry)){
		 $ctr1s++;
    	 if ($ctr1s==2){$bgclr1s='#ffffff';$ctr1s=0;} else { $bgclr1s='#F9ECCF';}
		$levtotalqry=mysql_query("SELECT * from employee_leave where employeeId='".$empid."' and leaveId='".$lev->ndex."' 
			and startdate>='".date('Y')."-01-01' and startdate<='".date('Y')."-12-31'
		 order by startDate");
		$levlimit=mysql_fetch_object(mysql_query("select * from employee_leave_limit where employeeId='".$empid."' and leaveId='".$lev->ndex."' and yer='".date('Y')."'"));
		$levconsume=0;
		$leavedates="";
...
if($_GET['ser']){	
	...
	while($pp=mysql_fetch_object($o)){
		...
		$da.="<a href='#' onclick=\"window.location.href='tools_setshiftingperdept.php?emp=".$pp->ndex."&startDate=".$_GET['startDate']."&endDate=".$_GET['endDate']."'\"><font color='blue'>".getID($pp->employmentStatus,$pp->employeeNo)." - ".$pp->lastName.", ".$pp->firstName." - ".$d->name."<br></font></a>";
	}
	echo $da;
}
else{
...
while($rsleave2=mysql_fetch_object($leave2)){
	$optionsh.="<option value='l".$rsleave2->ndex."'>".$rsleave2->code."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$rsleave2->name." (".getleavebal($_GET['emp'],$rsleave2->ndex).")";
}
		...
		if($ndx>=1 || $ndx=='FF'){
			...
			elseif(substr($var,0,1)=='l'){
				...
				$ins=mysql_query("insert into employee_leave (`employeeId`, `leaveId`, `startDate`, `endDate`)
					...
					$errl = '<h2 style="color:red;">Note: VL was not saved due to insufficient leave balance!</h2>';
				}
			}
...
<div id="main_content_wrap" class="container_12">
     <h2>Employee Shifting Schedule</h2>   
    <div class="clearfix">
...
<table>
	<tr>
		<td colspan="2" style="color:maroon;font-weight:bold;font-size:16px;letter-spacing:2px"><u><?php echo getID($emp->employmentStatus,$emp->employeeNo)." - ".$emp->lastName.", ".$emp->firstName." ".$emp->middleName;?></u><br><br></td>
	</tr>
