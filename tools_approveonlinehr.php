...
if($_GET['act']=='aprob'){
	$aqry="SELECT k.*,e.firstName,e.lastName,e.middleName,p.name as position,d.name as dept,e.ndex as empid,e.employmentStatus,e.employeeNo from kiosk_request k left join employee e on e.ndex=k.empid left join position p on p.ndex=e.position left join dept d on d.ndex=e.deptId";
	...
	while($ar=mysql_fetch_array($aexec)){
		...
		if($_POST['ck'.$jj]=="on"){
			...
			if($ar['tayp']=='drd'){
				...
				$ckkk = mysql_fetch_array(mysql_query("select * from dailytimesummary where employeeId=".$ar['empid']." and date='".$ar['date']."'"));
				if($ckkk['ndex']){
				$upd=mysql_query("update dailytimesummary set hoursDuty='".$hours."',drdRemarks='".$remarks."',otRestDay='".$ot."',night_prem='".$np."',overtimeRemarks='".$remarks."'
				 where employeeId=".$ar['empid']." and date='".$ar['date']."'");				
					...
					die("Please Process ".$ar['date']." first!");
				}
			...
			if($ar['tayp']=='Schedule'){
				if($ar['request'] == 'OFF'){
					...
					$ch = mysql_fetch_array(mysql_query("select * from employee_restday where employeeId='".$_GET['emp']."'
							and restday='".$daylog."' and startDate='".$ar['date']."'
						"));
					...
					$ch = mysql_fetch_array(mysql_query("select * from employee_shifting where employeeId='".$_GET['emp']."'
							and shiftingId='".str_replace('s', '', $ar['request'])."' and startDate='".$ar['date']."'
						"));
	...
	$daysoption='';
	for ( $a = $start; $a <= $end; $a += 86400 ){
		$daysoption.='<option value="'.date('Ymd',$a).'">'.date('Y-m-d',$a).'</option>';
	}
	...
	foreach($types as $type){
		...
		$qry.=" where k.approve1=1 and k.tayp='".$type."' and k.approve2='".$_GET['aStatus']."'
			and k.deptApprovedDate>='".$_GET['startDate']."' and k.deptApprovedDate<='".$_GET['endDate']." 23:59:59' and k.isPending=0
		";
		...
		while($r=mysql_fetch_object($exec)){
			...
			if($r->approve2==1){
				...
				$is_cb = '
							...
							<a title="Forward to Pending"  onclick="showtr('.$r->ndex.');" class="btn btn-sm red"> <i class="fa fa-mail-forward"></i></a>
				';
			}
			...
			if($type=='Overtime'){
    			...
    			$outlog=mysql_fetch_object(mysql_query("select * from hrinterface where dtrid='".$r->biometricNo."'  and log>'".$inlog->log."' and in_out=1 ORDER BY log LIMIT 0,1"));
    			$exp = explode("|",$r->request);
    			/*
    			$r->request = "
    				Overtime = ".$exp[0]." hrs<br>
    				Night Premium = ".$exp[1]." hrs<br>
    				Time In = ".date('F d, H:i:s',strtotime($inlog->log))." <br>
    				Time Out = ".date('F d, H:i:s',strtotime($outlog->log))." <br>
    			...
    			*/
    			...
    			";
    			$ot_ti = date('M d, H:i:s',strtotime($inlog->log))." <br> ".date('M d, H:i:s',strtotime($outlog->log));
				$ot_si = $getshift->timeIn." to ".$getshift->timeOut;
			...
			if($log['ndex']){
			    ...
			    $class=' class="dataf '.date('Ymd',strtotime($log['timelog'])).'"';
			  }
			  else{
			    $date_approved='';
			    $class=' class="dataf "';
			...
			$data[$type].="
				<tr style='color:black;' id='cdiv".$r->ndex."' ".$class.">
				<td id='cbdiv".$r->ndex."'>".$is_cb."<br>".$date_approved_hr."</td>
				<td>".$date_created."</td>
				<td>".$date_approved."</td>
				<td><a href='#\' onclick=\"window.open('employeelogs.php?id=".$r->empid."','displayWindow','toolbar=no,scrollbars=yes,width=1200,height=500')\";> ".getID($er->employmentStatus,$r->employeeNo)."</a></td>
			...
			$data[$type].='</tr>
			<tr style="display:none;" id="tr'.$r->ndex.'"><td colspan="11">
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							
							<h4 class="modal-title">Forward to Pending List</h4>							
						</div>			
						<div class="modal-body">
							<textarea placeholder="Remarks.." class="form-control" id="modalremark'.$r->ndex.'"></textarea>
						</div>
						<div class="modal-footer">
							<a href="#" class="btn default" onclick="hidetr('.$r->ndex.')";>Close</a>
							<a onclick="pending_req('.$r->ndex.')" class="btn green">Submit</a>
						</div>
					</div>		
				</div>	
			</td></tr>
...
<title>DTR System - Davao Doctors Hospital</title>
  ...
  <link href="kiosk1/assets/global/plugins/font-awesome/css/font-awesome.min.css" rel="stylesheet">
	...
	<link href="kiosk1/assets/global/plugins/uniform/css/uniform.default.css" rel="stylesheet">
	<link href="kiosk1/assets/global/plugins/bootstrap-switch/css/bootstrap-switch.min.css" rel="stylesheet">
  <!-- Theme styles START -->
...
<div id="rcont">
  ...
  <h2><strong>Approve Applications</strong> <a href="tools_approveonlinehr_pending.php" target="_blank" class="btn red">Pending Request</a></h2>
			<div id="overDiv" style="position:absolute;right: 220px; top: 200px; visibility:hidden; z-index:1000;"></div>
			<?php include "calendar.inc"; ?>
			
			
				<form action="tools_approveonlinehr.php" method="get" name="sadsa">
							...
							<td colspan="5">
								<table><tr>
									...
									<td>Select Dept: <select name="depsel" style="width:150px;"><option value="all">-All Assigned Dept-<?php echo $optiondepts;?></select></td>
										...
										<td>Filter Date:<select name="dayf" id="dayf" onchange="filter()"><option>Dates</option><?php echo $daysoption;?>
					        ...
					        <div class="col-md-12 tab-style-1">
						          ...
						          <div class="tab-content">
						            <div id="tab-1" class="tab-pane row fade active in">              
						              <div class="col-md-12">
						              	<form name="frmaproveshfting" action="tools_approveonlinehr.php?type=Overtime&act=aprob&startDate=<?php echo $_GET['startDate'];?>&endDate=<?php echo $_GET['endDate'];?>&depsel=<?php echo $_GET['depsel'];?>" method="post">
						            ...
						            <div id="tab-2" class="tab-pane row fade">              
						              <div class="col-md-12">
						              	<form name="frmaproveshfting" action="tools_approveonlinehr.php?type=drd&act=aprob&startDate=<?php echo $_GET['startDate'];?>&endDate=<?php echo $_GET['endDate'];?>&depsel=<?php echo $_GET['depsel'];?>" method="post">
											<input type="Hidden" name="act" value="aprob">
											<input type="Hidden" name="startDate" value="<?php echo $_GET['startDate'];?>">
											<input type="Hidden" name="endDate" value="<?php echo $_GET['endDate'];?>">
											<input type="Hidden" name="depdep" value="<?php echo $dip;?>">
						            ...
						            <div id="tab-3" class="tab-pane row fade">              
						              <div class="col-md-12">
						              	...
						              	<form name="frmaproveshfting" action="tools_approveonlinehr.php?type=log&act=aprob&startDate=<?php echo $_GET['startDate'];?>&endDate=<?php echo $_GET['endDate'];?>&depsel=<?php echo $_GET['depsel'];?>" method="post">
											<input type="Hidden" name="act" value="aprob">
											<input type="Hidden" name="startDate" value="<?php echo $_GET['startDate'];?>">
											<input type="Hidden" name="endDate" value="<?php echo $_GET['endDate'];?>">
											<input type="Hidden" name="depdep" value="<?php echo $dip;?>">
						            ...
						            <div id="tab-4" class="tab-pane row fade">              
						              <div class="col-md-12">
						              	<form name="frmaproveshfting" action="tools_approveonlinehr.php?type=Schedule&act=aprob&startDate=<?php echo $_GET['startDate'];?>&endDate=<?php echo $_GET['endDate'];?>&depsel=<?php echo $_GET['depsel'];?>" method="post">
											<input type="Hidden" name="act" value="aprob">
											<input type="Hidden" name="startDate" value="<?php echo $_GET['startDate'];?>">
											<input type="Hidden" name="endDate" value="<?php echo $_GET['endDate'];?>">
											<input type="Hidden" name="depdep" value="<?php echo $dip;?>">
						            ...
						            <div id="tab-5" class="tab-pane row fade">              
						              <div class="col-md-12">
						              	<form name="frmaproveshfting" action="tools_approveonlinehr.php?type=leave&act=aprob&startDate=<?php echo $_GET['startDate'];?>&endDate=<?php echo $_GET['endDate'];?>&depsel=<?php echo $_GET['depsel'];?>" method="post">
											<input type="Hidden" name="act" value="aprob">
											<input type="Hidden" name="startDate" value="<?php echo $_GET['startDate'];?>">
											<input type="Hidden" name="endDate" value="<?php echo $_GET['endDate'];?>">
											<input type="Hidden" name="depdep" value="<?php echo $dip;?>">
...
<script src="kiosk1/assets/global/plugins/jquery-1.11.0.min.js" type="text/javascript"></script>
    ...
    function approve_req(x){
   	...
   	function filter(){
   		//alert(jQuery('#dayf').val());
   		jQuery('.dataf').show();
   		jQuery('.dataf').not('.'+jQuery('#dayf').val()).hide();
   	}

    function pending_req(x){
    	//var aa = ;
    	if(!jQuery('#modalremark'+x).val()){
    		...
    		jQuery.ajax({
	    		method: "POST",
	    		url: "tools_approveonlinehr_ajax.php?act=pending&id="+x+"&remark="+jQuery('#modalremark'+x).val()
	    	})
	    	.done(function(data){
	    		jQuery('#cbdiv'+x).html('Forwarded to Pending List');
	    		jQuery('#cdiv'+x).hide(1000,'swing');
	    		hidetr(x);
	    		//alert(data);
	    	});
